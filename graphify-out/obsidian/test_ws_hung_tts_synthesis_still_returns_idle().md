---
source_file: "gateway/tests/test_voice_gateway.py"
type: "code"
community: "asyncio"
location: "L2471"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/asyncio
---

# test_ws_hung_tts_synthesis_still_returns_idle()

## Connections
- [[A wedged TTS synthesis (e.g. blocked voice-pack download — live incident…]] - `rationale_for` [EXTRACTED]
- [[_hung_synthesize()]] - `indirect_call` [INFERRED]
- [[_mock_agent()_2]] - `indirect_call` [INFERRED]
- [[_mock_agent()_4]] - `contains` [EXTRACTED]
- [[_mock_ws()]] - `calls` [EXTRACTED]
- [[_pcm_bytes()]] - `calls` [EXTRACTED]
- [[asyncio_1]] - `references` [EXTRACTED]
- [[test_voice_gateway.py]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/asyncio