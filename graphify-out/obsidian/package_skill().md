---
source_file: "skills/openclaw/skill-creator/scripts/package_skill.py"
type: "code"
community: "Community 543"
location: "L20"
tags:
  - graphify/code
  - graphify/INFERRED
  - community/Community_543
---

# package_skill()

## Connections
- [[dot-test_allows_nested_regular_files()]] - `calls` [INFERRED]
- [[dot-test_packages_normal_files()]] - `calls` [INFERRED]
- [[dot-test_rejects_symlink_directory()]] - `calls` [INFERRED]
- [[dot-test_rejects_symlink_to_external_file()]] - `calls` [INFERRED]
- [[Package a skill folder into a .skill file.      Args         skill_path Path t]] - `rationale_for` [EXTRACTED]
- [[init_skill.py]] - `conceptually_related_to` [INFERRED]
- [[main()_25]] - `calls` [EXTRACTED]
- [[package_skill.py]] - `contains` [EXTRACTED]
- [[validate_skill()]] - `calls` [INFERRED]

#graphify/code #graphify/INFERRED #community/Community_543