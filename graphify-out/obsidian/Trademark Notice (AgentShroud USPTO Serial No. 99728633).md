---
source_file: "docs/project/legal/TRADEMARK.md"
type: "document"
community: "Module Group 399"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/Module_Group_399
---

# Trademark Notice (AgentShroud USPTO Serial No. 99728633)

## Connections
- [[AgentShroud Communication Templates (Trademark Statements)]] - `references` [EXTRACTED]
- [[AgentShroud Trademark Prior Use Record]] - `supports` [INFERRED]
- [[Trademark Usage Guidelines (MayMay Not Use)]] - `contains` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/Module_Group_399
