---
type: community
cohesion: 0.24
members: 12
---

# Module Group 328

**Cohesion:** 0.24 - loosely connected
**Members:** 12 nodes

## Members
- [[._async_send()]] - code - gateway/proxy/telegram_egress_notify.py
- [[.answer_callback()]] - code - gateway/proxy/telegram_egress_notify.py
- [[.edit_decision_message()]] - code - gateway/proxy/telegram_egress_notify.py
- [[Async wrapper around sync Telegram API call.]] - rationale - gateway/proxy/telegram_egress_notify.py
- [[BaseException_1]] - code - gateway/proxy/telegram_egress_notify.py
- [[Replace the inline keyboard approval message with a decision record.          Re]] - rationale - gateway/proxy/telegram_egress_notify.py
- [[Send answerCallbackQuery to dismiss the button loading state.          Pass ``to]] - rationale - gateway/proxy/telegram_egress_notify.py
- [[_err_text()]] - code - gateway/proxy/telegram_egress_notify.py
- [[_is_stale_callback_error()]] - code - gateway/proxy/telegram_egress_notify.py
- [[_is_stale_edit_error()]] - code - gateway/proxy/telegram_egress_notify.py
- [[telegram_egress_notify.py]] - code - gateway/proxy/telegram_egress_notify.py
- [[urllib HTTPError carries the response body on .read(); fall back to str.]] - rationale - gateway/proxy/telegram_egress_notify.py

## Live Query (requires Dataview plugin)

```dataview
TABLE source_file, type FROM #community/Module_Group_328
SORT file.name ASC
```

## Connections to other communities
- 5 edges to [[_COMMUNITY_Module Group 279]]
- 4 edges to [[_COMMUNITY_Module Group 410]]

## Top bridge nodes
- [[_is_stale_callback_error()]] - degree 6, connects to 1 community
- [[_is_stale_edit_error()]] - degree 6, connects to 1 community
- [[._async_send()]] - degree 5, connects to 1 community
- [[telegram_egress_notify.py]] - degree 4, connects to 1 community
- [[.answer_callback()]] - degree 4, connects to 1 community
