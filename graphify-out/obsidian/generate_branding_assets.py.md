---
source_file: "scripts/generate_branding_assets.py"
type: "code"
community: "Module Group 219"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Module_Group_219
---

# generate_branding_assets.py

## Connections
- [[ensure()]] - `contains` [EXTRACTED]
- [[generate_email()]] - `contains` [EXTRACTED]
- [[generate_favicons()]] - `contains` [EXTRACTED]
- [[generate_feature_icons()]] - `contains` [EXTRACTED]
- [[generate_icon_sizes()]] - `contains` [EXTRACTED]
- [[generate_presentation()]] - `contains` [EXTRACTED]
- [[generate_social()]] - `contains` [EXTRACTED]
- [[generate_svg_logos()]] - `contains` [EXTRACTED]
- [[generate_variants()]] - `contains` [EXTRACTED]
- [[get_font()]] - `contains` [EXTRACTED]
- [[icon_on_canvas()]] - `contains` [EXTRACTED]
- [[load_logo()]] - `contains` [EXTRACTED]
- [[main()_6]] - `contains` [EXTRACTED]
- [[save()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Module_Group_219