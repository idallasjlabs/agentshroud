---
source_file: "docs/architecture/WORKSPACE_USAGE.md"
type: "document"
community: ".mcp.json"
location: "L1"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/mcpjson
---

# OpenClaw Workspace Usage Guide

## Connections
- [[Advanced Direct Docker Commands]] - `contains` [EXTRACTED]
- [[Backup and Restore]] - `contains` [EXTRACTED]
- [[Common Workflows_1]] - `contains` [EXTRACTED]
- [[File Permissions]] - `contains` [EXTRACTED]
- [[Integration with Bot]] - `contains` [EXTRACTED]
- [[Quick Reference_3]] - `contains` [EXTRACTED]
- [[Summary_21]] - `contains` [EXTRACTED]
- [[Troubleshooting_12]] - `contains` [EXTRACTED]
- [[WORKSPACE_USAGE]] - `contains` [EXTRACTED]
- [[What is the Workspace]] - `contains` [EXTRACTED]
- [[Why Docker Volume vs Bind Mount]] - `contains` [EXTRACTED]
- [[Workspace Decision Docker Volume (not bind mount)]] - `implements` [EXTRACTED]
- [[Workspace Helper Script dockerscriptsworkspace.sh — ls, cp-to, cp-from, cat, mkdir, rm, shell, tree]] - `documents` [EXTRACTED]
- [[Workspace Organization Tips]] - `contains` [EXTRACTED]
- [[Workspace Size]] - `contains` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/mcpjson