---
source_file: "gateway/tests/test_group_config.py"
type: "code"
community: "Group Config & Teams"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Group_Config__Teams
---

# test_group_config.py

## Connections
- [[GroupConfig]] - `imports` [EXTRACTED]
- [[ProjectConfig]] - `imports` [EXTRACTED]
- [[TeamsConfig]] - `imports` [EXTRACTED]
- [[TestAdminChecks]] - `contains` [EXTRACTED]
- [[TestCollabMode]] - `contains` [EXTRACTED]
- [[TestGroupSafeResponsePrefix]] - `contains` [EXTRACTED]
- [[TestMembershipQueries]] - `contains` [EXTRACTED]
- [[TestProjectQueries]] - `contains` [EXTRACTED]
- [[TestTeamsConfigParsing]] - `contains` [EXTRACTED]
- [[TestUserCollabModeOverride]] - `contains` [EXTRACTED]
- [[persist_user_collab_mode()]] - `imports` [EXTRACTED]
- [[teams()_1]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Group_Config__Teams