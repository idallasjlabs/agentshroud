---
source_file: "docker/config/hermes/skills/i-aws/SKILL.md"
type: "document"
community: "Bot Skill Config"
location: "L6"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/Bot_Skill_Config
---

# AWS Cloud Management & FinOps Agent

## Connections
- [[.scripts Numbered Script Library Convention]] - `implements` [EXTRACTED]
- [[AWS Cloud Management & FinOps README]] - `references` [INFERRED]
- [[AWS Tagging Standard]] - `implements` [EXTRACTED]
- [[Audit-log-every-fetch design rationale]] - `semantically_similar_to` [INFERRED]
- [[Cross-Account & Future Proofing]] - `contains` [EXTRACTED]
- [[Cross-Account & Future Proofing_1]] - `contains` [EXTRACTED]
- [[Decision Framework]] - `contains` [EXTRACTED]
- [[Decision Framework_1]] - `contains` [EXTRACTED]
- [[Deliverables]] - `contains` [EXTRACTED]
- [[Deliverables_1]] - `contains` [EXTRACTED]
- [[EBS Performance Analysis Pattern]] - `implements` [EXTRACTED]
- [[Expertise]] - `contains` [EXTRACTED]
- [[Expertise_1]] - `contains` [EXTRACTED]
- [[FY26 Cost Reduction Context]] - `contains` [EXTRACTED]
- [[FY26 Cost Reduction Context_1]] - `contains` [EXTRACTED]
- [[FY26 Cost Reduction Plan (40% target)]] - `implements` [EXTRACTED]
- [[GSDE&G Team (Global Services Digital Enablement & Governance)]] - `references` [EXTRACTED]
- [[Guardrails_1]] - `contains` [EXTRACTED]
- [[Guardrails_6]] - `contains` [EXTRACTED]
- [[Identity_1]] - `contains` [EXTRACTED]
- [[Identity_2]] - `contains` [EXTRACTED]
- [[Operating Rules (Non-Negotiable)]] - `contains` [EXTRACTED]
- [[Operating Rules (Non-Negotiable)_1]] - `contains` [EXTRACTED]
- [[Resource Inventory CSV Schema]] - `contains` [EXTRACTED]
- [[Resource Inventory CSV Schema_1]] - `contains` [EXTRACTED]
- [[SKILL_7]] - `contains` [EXTRACTED]
- [[SKILL_70]] - `contains` [EXTRACTED]
- [[Tagging Standard]] - `contains` [EXTRACTED]
- [[Tagging Standard_1]] - `contains` [EXTRACTED]
- [[Workflow]] - `contains` [EXTRACTED]
- [[Workflow_2]] - `contains` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/Bot_Skill_Config