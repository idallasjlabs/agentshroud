---
source_file: "gateway/tests/test_scanner_integration.py"
type: "code"
community: "Scanner Integration Tests"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Scanner_Integration_Tests
---

# test_scanner_integration.py

## Connections
- [[TestAggregateResults]] - `contains` [EXTRACTED]
- [[TestComputeScorecard]] - `contains` [EXTRACTED]
- [[TestGetClamavSummary]] - `contains` [EXTRACTED]
- [[TestGetFalcoSummary]] - `contains` [EXTRACTED]
- [[TestGetOpenscapSummary]] - `contains` [EXTRACTED]
- [[TestGetSbom]] - `contains` [EXTRACTED]
- [[TestGetTrivyImageSummaries]] - `contains` [EXTRACTED]
- [[TestGetTrivySummary]] - `contains` [EXTRACTED]
- [[TestGetWazuhSummary]] - `contains` [EXTRACTED]
- [[TestLoadLatestJson]] - `contains` [EXTRACTED]
- [[TestScoreComplianceAuditing]] - `contains` [EXTRACTED]
- [[TestScoreContainerHardening]] - `contains` [EXTRACTED]
- [[TestScoreImageIntegrity]] - `contains` [EXTRACTED]
- [[TestScoreIncidentResponse]] - `contains` [EXTRACTED]
- [[TestScoreLoggingMonitoring]] - `contains` [EXTRACTED]
- [[TestScoreMalwareDefense]] - `contains` [EXTRACTED]
- [[TestScoreNetworkSegmentation]] - `contains` [EXTRACTED]
- [[TestScoreRuntimeProtection]] - `contains` [EXTRACTED]
- [[TestScoreSecretsManagement]] - `contains` [EXTRACTED]
- [[TestScoreSecureDevelopment]] - `contains` [EXTRACTED]
- [[TestScoreSupplyChain]] - `contains` [EXTRACTED]
- [[TestScoreVulnerabilityManagement]] - `contains` [EXTRACTED]
- [[_clamav_clean()]] - `contains` [EXTRACTED]
- [[_clamav_infected()]] - `contains` [EXTRACTED]
- [[_clean_trivy_report()]] - `contains` [EXTRACTED]
- [[_compute_weighted_subscore()]] - `imports` [EXTRACTED]
- [[_critical_trivy_report()]] - `contains` [EXTRACTED]
- [[_falco_clean()]] - `contains` [EXTRACTED]
- [[_falco_critical()]] - `contains` [EXTRACTED]
- [[_falco_not_run()]] - `contains` [EXTRACTED]
- [[_load_latest_json()]] - `imports` [EXTRACTED]
- [[_openscap_clean()]] - `contains` [EXTRACTED]
- [[_openscap_not_run()]] - `contains` [EXTRACTED]
- [[_openscap_warn()]] - `contains` [EXTRACTED]
- [[_score_compliance_auditing()]] - `imports` [EXTRACTED]
- [[_score_container_hardening()]] - `imports` [EXTRACTED]
- [[_score_image_integrity()]] - `imports` [EXTRACTED]
- [[_score_incident_response()]] - `imports` [EXTRACTED]
- [[_score_logging_monitoring()]] - `imports` [EXTRACTED]
- [[_score_malware_defense()]] - `imports` [EXTRACTED]
- [[_score_network_segmentation()]] - `imports` [EXTRACTED]
- [[_score_runtime_protection()]] - `imports` [EXTRACTED]
- [[_score_secrets_management()]] - `imports` [EXTRACTED]
- [[_score_secure_development()]] - `imports` [EXTRACTED]
- [[_score_supply_chain()]] - `imports` [EXTRACTED]
- [[_score_vulnerability_management()]] - `imports` [EXTRACTED]
- [[_trivy_clean()]] - `contains` [EXTRACTED]
- [[_trivy_critical()]] - `contains` [EXTRACTED]
- [[_trivy_not_run()]] - `contains` [EXTRACTED]
- [[_wazuh_clean()]] - `contains` [EXTRACTED]
- [[_wazuh_not_run()]] - `contains` [EXTRACTED]
- [[_write_image_report()]] - `contains` [EXTRACTED]
- [[aggregate_results()]] - `imports` [EXTRACTED]
- [[compute_scorecard()]] - `imports` [EXTRACTED]
- [[get_clamav_summary()]] - `imports` [EXTRACTED]
- [[get_falco_summary()]] - `imports` [EXTRACTED]
- [[get_openscap_summary()]] - `imports` [EXTRACTED]
- [[get_sbom()]] - `imports` [EXTRACTED]
- [[get_trivy_image_summaries()]] - `imports` [EXTRACTED]
- [[get_trivy_summary()]] - `imports` [EXTRACTED]
- [[get_wazuh_summary()]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Scanner_Integration_Tests