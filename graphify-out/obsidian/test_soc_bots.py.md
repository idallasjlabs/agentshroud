---
source_file: "gateway/tests/test_soc_bots.py"
type: "code"
community: "Voice Gateway Tests"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Voice_Gateway_Tests
---

# test_soc_bots.py

## Connections
- [[RBACConfig_1]] - `imports` [EXTRACTED]
- [[Role_1]] - `imports` [EXTRACTED]
- [[SCLCaller]] - `imports` [EXTRACTED]
- [[SecurityEvent_1]] - `imports` [EXTRACTED]
- [[ServiceDescriptor]] - `imports` [EXTRACTED]
- [[ServiceStatus]] - `imports` [EXTRACTED]
- [[Severity_2]] - `imports` [EXTRACTED]
- [[TestAgentCvesBotId]] - `contains` [EXTRACTED]
- [[TestBotSelectorFrontend]] - `contains` [EXTRACTED]
- [[TestCollaboratorActivityBotFilter]] - `contains` [EXTRACTED]
- [[TestComputeBotScorecard_1]] - `contains` [EXTRACTED]
- [[TestConfigBotId]] - `contains` [EXTRACTED]
- [[TestEgressHistoryBotFilter]] - `contains` [EXTRACTED]
- [[TestEgressLogBotFilter]] - `contains` [EXTRACTED]
- [[TestEgressPendingBotFilter]] - `contains` [EXTRACTED]
- [[TestListBots]] - `contains` [EXTRACTED]
- [[TestScannersBotId]] - `contains` [EXTRACTED]
- [[TestScorecardBotId]] - `contains` [EXTRACTED]
- [[TestSecurityEventsBotFilter]] - `contains` [EXTRACTED]
- [[TestServicesBotFilter]] - `contains` [EXTRACTED]
- [[_make_app_state()]] - `contains` [EXTRACTED]
- [[_make_bot_config()]] - `contains` [EXTRACTED]
- [[_make_m6_app_state()]] - `contains` [EXTRACTED]
- [[_make_m6_bot_config()]] - `contains` [EXTRACTED]
- [[_make_m6_caller()]] - `contains` [EXTRACTED]
- [[_make_owner_caller()]] - `contains` [EXTRACTED]
- [[compute_bot_scorecard()]] - `imports` [EXTRACTED]
- [[get_agent_cves()]] - `imports` [EXTRACTED]
- [[get_collaborator_activity()]] - `imports` [EXTRACTED]
- [[get_config()]] - `imports` [EXTRACTED]
- [[get_egress_history()]] - `imports` [EXTRACTED]
- [[get_egress_log()]] - `imports` [EXTRACTED]
- [[get_egress_pending()_1]] - `imports` [EXTRACTED]
- [[get_scanner_results()]] - `imports` [EXTRACTED]
- [[get_security_events()]] - `imports` [EXTRACTED]
- [[get_security_scorecard()]] - `imports` [EXTRACTED]
- [[list_bots()]] - `imports` [EXTRACTED]
- [[list_services()]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Voice_Gateway_Tests