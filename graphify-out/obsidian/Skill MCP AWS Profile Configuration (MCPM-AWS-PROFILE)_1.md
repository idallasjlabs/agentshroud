---
source_file: "docker/config/openclaw/skills/i-mcpm-aws-profile/SKILL.md"
type: "document"
community: "Bot Skill Config"
location: "L5"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/Bot_Skill_Config
---

# Skill: MCP AWS Profile Configuration (MCPM-AWS-PROFILE)

## Connections
- [[AWS Profile Basics_1]] - `contains` [EXTRACTED]
- [[AWS SSO Configuration_1]] - `contains` [EXTRACTED]
- [[Best Practices_3]] - `contains` [EXTRACTED]
- [[Configuration Files_1]] - `contains` [EXTRACTED]
- [[Configure New Profile_1]] - `contains` [EXTRACTED]
- [[List Available Profiles_1]] - `contains` [EXTRACTED]
- [[Multi-Account AWS Access_1]] - `contains` [EXTRACTED]
- [[Quick Reference_1]] - `contains` [EXTRACTED]
- [[Related Skills_59]] - `contains` [EXTRACTED]
- [[Role_53]] - `contains` [EXTRACTED]
- [[SKILL_95]] - `contains` [EXTRACTED]
- [[Switch AWS Profile for MCP_1]] - `contains` [EXTRACTED]
- [[Troubleshooting_10]] - `contains` [EXTRACTED]
- [[When to Invoke_4]] - `contains` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/Bot_Skill_Config