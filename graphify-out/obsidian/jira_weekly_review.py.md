---
source_file: "docker/config/hermes/workspace/jira_weekly_review.py"
type: "code"
community: "Community 245"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_245
---

# jira_weekly_review.py

## Connections
- [[Jira Dev-Ticket Helper Module]] - `references` [EXTRACTED]
- [[_adf_paragraph()]] - `contains` [EXTRACTED]
- [[_git_commits_last_week()]] - `contains` [EXTRACTED]
- [[_http_get()]] - `contains` [EXTRACTED]
- [[_http_post_json()]] - `contains` [EXTRACTED]
- [[build_basic_auth_header()_1]] - `contains` [EXTRACTED]
- [[build_comment_payload()_1]] - `contains` [EXTRACTED]
- [[build_comment_url()_1]] - `contains` [EXTRACTED]
- [[build_op_proxy_request()_1]] - `contains` [EXTRACTED]
- [[build_tenant_info_url()_1]] - `contains` [EXTRACTED]
- [[build_weekly_summary()]] - `contains` [EXTRACTED]
- [[datetime]] - `imports_from` [EXTRACTED]
- [[extract_scrum_items()]] - `contains` [EXTRACTED]
- [[fetch_op_secret()_1]] - `contains` [EXTRACTED]
- [[post_comment()]] - `contains` [EXTRACTED]
- [[resolve_cloud_id()_1]] - `contains` [EXTRACTED]
- [[run()_1]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_245