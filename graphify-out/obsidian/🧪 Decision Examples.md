---
source_file: "docker/SYSTEM-INSTRUCTIONS-SECURITY.md"
type: "document"
community: "docs/api"
location: "L171"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/docs/api
---

# 🧪 Decision Examples

## Connections
- [[Example 1 User Asks for Password (Chat)]] - `contains` [EXTRACTED]
- [[Example 2 User Asks for Password (Console)]] - `contains` [EXTRACTED]
- [[Example 3 User Wants Service Configured]] - `contains` [EXTRACTED]
- [[Example 4 User Insists on Seeing Password]] - `contains` [EXTRACTED]
- [[System Instructions Credential Security (Ultra-Conservative)]] - `contains` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/docs/api