---
source_file: "docker/config/hermes/skills/i-browser/scripts/browse.py"
type: "code"
community: "Community 253"
location: "browse.py:245"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_253
---

# SecureBrowser.navigate()

## Connections
- [[Browser — Secure Browser Automation (SKILL)]] - `references` [EXTRACTED]
- [[SecureBrowser._classify_risk()]] - `calls` [EXTRACTED]
- [[SecureBrowser._init_browser()]] - `calls` [EXTRACTED]
- [[SecureBrowser._log_action()]] - `calls` [EXTRACTED]
- [[SecureBrowser._take_screenshot()]] - `calls` [EXTRACTED]
- [[SecureBrowser._validate_url()]] - `calls` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_253