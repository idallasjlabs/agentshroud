---
source_file: "skills/openclaw/peekaboo/SKILL.md"
type: "document"
community: "Community 575"
location: "L116"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/Community_575
---

# Examples

## Connections
- [[App + window management]] - `contains` [EXTRACTED]
- [[Capture screenshots + analyze]] - `contains` [EXTRACTED]
- [[Keyboard input]] - `contains` [EXTRACTED]
- [[Live capture (motion-aware)]] - `contains` [EXTRACTED]
- [[Menus, menubar, dock]] - `contains` [EXTRACTED]
- [[Mouse + gesture input]] - `contains` [EXTRACTED]
- [[Peekaboo]] - `contains` [EXTRACTED]
- [[See - click - type (most reliable flow)]] - `contains` [EXTRACTED]
- [[Target by window id]] - `contains` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/Community_575