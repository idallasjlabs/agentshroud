---
source_file: "voice_gateway/tts.py"
type: "code"
community: "Community 511"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_511
---

# tts.py

## Connections
- [[_get_pipeline()]] - `contains` [EXTRACTED]
- [[_resample_s16le_mono()]] - `contains` [EXTRACTED]
- [[normalize_for_speech()]] - `contains` [EXTRACTED]
- [[server.py]] - `imports_from` [EXTRACTED]
- [[split_for_speech()]] - `contains` [EXTRACTED]
- [[synthesize()]] - `contains` [EXTRACTED]
- [[test_voice_gateway.py]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_511