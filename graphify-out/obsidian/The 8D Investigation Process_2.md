---
source_file: "docker/config/openclaw/skills/i-eightd/SKILL.md"
type: "document"
community: "Community 237"
location: "L58"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/Community_237
---

# The 8D Investigation Process

## Connections
- [[8D Root Cause Analysis — Data-Driven Control System Investigation_2]] - `contains` [EXTRACTED]
- [[D0 — Planning & Scoping_2]] - `contains` [EXTRACTED]
- [[D1 — Team & Expertise Identification_2]] - `contains` [EXTRACTED]
- [[D2 — Problem Description (Data Collection & Characterization)_2]] - `contains` [EXTRACTED]
- [[D3 — Interim Containment Assessment_2]] - `contains` [EXTRACTED]
- [[D4 — Root Cause Analysis (Correlated Event Detection)_2]] - `contains` [EXTRACTED]
- [[D5 — Permanent Corrective Action Definition_2]] - `contains` [EXTRACTED]
- [[D6 — Implementation Verification_2]] - `contains` [EXTRACTED]
- [[D7 — Systemic Prevention_2]] - `contains` [EXTRACTED]
- [[D8 — Documentation & Recognition_2]] - `contains` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/Community_237