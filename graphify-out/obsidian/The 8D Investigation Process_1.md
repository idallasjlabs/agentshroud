---
source_file: "docker/config/hermes/skills/i-eightd/SKILL.md"
type: "document"
community: "Community 228"
location: "L58"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/Community_228
---

# The 8D Investigation Process

## Connections
- [[8D Root Cause Analysis — Data-Driven Control System Investigation_1]] - `contains` [EXTRACTED]
- [[D0 — Planning & Scoping_1]] - `contains` [EXTRACTED]
- [[D1 — Team & Expertise Identification_1]] - `contains` [EXTRACTED]
- [[D2 — Problem Description (Data Collection & Characterization)_1]] - `contains` [EXTRACTED]
- [[D3 — Interim Containment Assessment_1]] - `contains` [EXTRACTED]
- [[D4 — Root Cause Analysis (Correlated Event Detection)_1]] - `contains` [EXTRACTED]
- [[D5 — Permanent Corrective Action Definition_1]] - `contains` [EXTRACTED]
- [[D6 — Implementation Verification_1]] - `contains` [EXTRACTED]
- [[D7 — Systemic Prevention_1]] - `contains` [EXTRACTED]
- [[D8 — Documentation & Recognition_1]] - `contains` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/Community_228