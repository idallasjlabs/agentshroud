---
source_file: "docker/falco/rules.yaml"
type: "document"
community: "Community 323"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/Community_323
---

# AgentShroud Falco Detection Rules

## Connections
- [[Falco Configuration]] - `references` [EXTRACTED]
- [[Falco Rule Container Shell Spawned]] - `references` [EXTRACTED]
- [[Falco Rule Crypto Mining Detection]] - `references` [EXTRACTED]
- [[Falco Rule File Access Outside Workspace]] - `references` [EXTRACTED]
- [[Falco Rule Privilege Escalation Attempt]] - `references` [EXTRACTED]
- [[Falco Rule Secret File Access]] - `references` [EXTRACTED]
- [[Falco Rule Unexpected Outbound Connection from AgentShroud]] - `references` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/Community_323