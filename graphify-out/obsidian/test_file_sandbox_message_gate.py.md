---
source_file: "gateway/tests/test_file_sandbox_message_gate.py"
type: "code"
community: "Session Manager & Webhook"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Session_Manager__Webhook
---

# test_file_sandbox_message_gate.py

## Connections
- [[MiddlewareManager]] - `imports` [EXTRACTED]
- [[MiddlewareResult]] - `imports` [EXTRACTED]
- [[TestFileSandboxSkippedForPlainMessages]] - `contains` [EXTRACTED]
- [[TestIsToolCallRequest]] - `contains` [EXTRACTED]
- [[TestOwnerBypassContentPatternChecks]] - `contains` [EXTRACTED]
- [[UserSessionManager]] - `imports` [EXTRACTED]
- [[_plain_msg()]] - `contains` [EXTRACTED]
- [[_tool_call_msg()]] - `contains` [EXTRACTED]
- [[_tool_result_msg()]] - `contains` [EXTRACTED]
- [[manager()]] - `contains` [EXTRACTED]
- [[session_manager()]] - `contains` [EXTRACTED]
- [[temp_workspace()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Session_Manager__Webhook
