---
source_file: "docs/architecture/agentic-os.md"
type: "document"
community: "Architecture Docs"
location: "L1"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/Architecture_Docs
---

# AgentShroud Agentic OS

## Connections
- [[1. System Overview]] - `contains` [EXTRACTED]
- [[10. Infrastructure & Runtime]] - `contains` [EXTRACTED]
- [[11. Automated Operations]] - `contains` [EXTRACTED]
- [[12. MCP Integrations]] - `contains` [EXTRACTED]
- [[13. Compliance Alignment]] - `contains` [EXTRACTED]
- [[14. Gap Analysis & Recommendations]] - `contains` [EXTRACTED]
- [[2. Multi-Agent Hierarchy]] - `contains` [EXTRACTED]
- [[3. Agent Inventory]] - `contains` [EXTRACTED]
- [[4. Skill System]] - `contains` [EXTRACTED]
- [[5. Orchestration Flow]] - `contains` [EXTRACTED]
- [[7. CICD Quality Gates]] - `contains` [EXTRACTED]
- [[8. Governance Model]] - `contains` [EXTRACTED]
- [[9. Human-in-the-Loop Controls]] - `contains` [EXTRACTED]
- [[AI-Native Development Framework]] - `contains` [EXTRACTED]
- [[Appendix A File Structure Reference]] - `contains` [EXTRACTED]
- [[Table of Contents]] - `contains` [EXTRACTED]
- [[`SecurityPipeline`]] - `contains` [EXTRACTED]
- [[agentic-os]] - `contains` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/Architecture_Docs