---
source_file: "docs/vault/02 - Modules/Proxy Layer/pipeline.py.md"
type: "code"
community: "Module Group 314"
location: "gateway/proxy/pipeline.py"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Module_Group_314
---

# pipeline.py — SecurityPipeline

## Connections
- [[AuditChain_1]] - `defines` [EXTRACTED]
- [[HTTPForwarder_1]] - `used_by` [EXTRACTED]
- [[LLMProxy_2]] - `depends_on` [EXTRACTED]
- [[PipelineResult_2]] - `defines` [EXTRACTED]
- [[SecurityPipeline_2]] - `defines` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Module_Group_314
