---
source_file: "docs/security/competitive-security-matrix.md"
type: "document"
community: "Bot Skill Config"
location: "L1"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/Bot_Skill_Config
---

# Competitive Security Matrix — AgentShroud vs AI Agent Platforms

## Connections
- [[Container Security Toolchain Comparison]] - `contains` [EXTRACTED]
- [[Key Takeaways]] - `contains` [EXTRACTED]
- [[Module Comparison]] - `contains` [EXTRACTED]
- [[Security Score (out of 28)]] - `contains` [EXTRACTED]
- [[Updated Magic Quadrant Position]] - `contains` [EXTRACTED]
- [[competitive-security-matrix]] - `contains` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/Bot_Skill_Config