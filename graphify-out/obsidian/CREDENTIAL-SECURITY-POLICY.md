---
source_file: "docs/security/CREDENTIAL-SECURITY-POLICY.md"
type: "document"
community: "Community 849"
location: "L1"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/Community_849
---

# CREDENTIAL-SECURITY-POLICY.md

## Connections
- [[CREDENTIAL-PROTECTION-IMPLEMENTED]] - `cites` [EXTRACTED]
- [[Credential Security Policy]] - `contains` [EXTRACTED]
- [[Option 1 Gateway-Level Filtering]] - `references` [EXTRACTED]
- [[Option 2 Approval Queue for Credential Ops]] - `references` [EXTRACTED]
- [[Option 3 Disable Credential Commands via Telegram]] - `references` [EXTRACTED]
- [[Option 4 Role-Based Access Control]] - `references` [EXTRACTED]
- [[SECURITY-POLICY-FINAL]] - `cites` [EXTRACTED]
- [[router.py]] - `references` [EXTRACTED]
- [[sanitizer.py]] - `references` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/Community_849