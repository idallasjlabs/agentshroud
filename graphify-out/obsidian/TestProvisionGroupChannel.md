---
source_file: "gateway/tests/test_slack_proxy_coverage.py"
type: "code"
community: "Slack Proxy & Main Endpoint Tests"
location: "L366"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Slack_Proxy__Main_Endpoint_Tests
---

# TestProvisionGroupChannel

## Connections
- [[dot-test_name_truncated_to_80_chars()]] - `method` [EXTRACTED]
- [[dot-test_no_token_returns_none()]] - `method` [EXTRACTED]
- [[dot-test_slack_error_returns_none()]] - `method` [EXTRACTED]
- [[dot-test_success_returns_channel_id_with_sanitized_name()]] - `method` [EXTRACTED]
- [[SlackAPIProxy_1]] - `uses` [INFERRED]
- [[test_slack_proxy_coverage.py]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Slack_Proxy__Main_Endpoint_Tests