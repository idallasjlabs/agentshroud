---
source_file: "docker/config/hermes/skills/i-ux/SKILL.md"
type: "document"
community: "Skill: UX Expert (UX)"
location: "L5"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/Skill_UX_Expert_UX
---

# Skill: UX Expert (UX)

## Connections
- [[Anti-Patterns to Flag_1]] - `contains` [EXTRACTED]
- [[CLI  TUI UX_1]] - `contains` [EXTRACTED]
- [[Core Discipline Discover → Define → Design → Validate_1]] - `contains` [EXTRACTED]
- [[Dashboard UX_1]] - `contains` [EXTRACTED]
- [[Dependencies_2]] - `contains` [EXTRACTED]
- [[Form UX_1]] - `contains` [EXTRACTED]
- [[Information Architecture_1]] - `contains` [EXTRACTED]
- [[Interaction Design Patterns_1]] - `contains` [EXTRACTED]
- [[Nielsen's 10 Heuristics — GSDE&G Application_1]] - `contains` [EXTRACTED]
- [[Role_19]] - `contains` [EXTRACTED]
- [[Rules_1]] - `contains` [EXTRACTED]
- [[Usability Audit Checklist_1]] - `contains` [EXTRACTED]
- [[User Flow Diagrams_1]] - `contains` [EXTRACTED]
- [[hermesskillsi-crSKILL]] - `contains` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/Skill_UX_Expert_UX