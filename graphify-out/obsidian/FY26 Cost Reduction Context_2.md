---
source_file: "docker/config/openclaw/skills/i-aws/SKILL.md"
type: "document"
community: "Community 229"
location: "L151"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/Community_229
---

# FY26 Cost Reduction Context

## Connections
- [[AWS Cloud Management & FinOps Agent_2]] - `contains` [EXTRACTED]
- [[In-Scope Departments (Cost Reduction)_2]] - `contains` [EXTRACTED]
- [[Infrastructure You Must Know_2]] - `contains` [EXTRACTED]
- [[Out-of-Scope Departments (Inventory & Tag Only)_2]] - `contains` [EXTRACTED]
- [[Savings Levers (Ranked by Impact)_2]] - `contains` [EXTRACTED]
- [[Target 40% Full-Year Reduction on Global Services Resources_2]] - `contains` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/Community_229