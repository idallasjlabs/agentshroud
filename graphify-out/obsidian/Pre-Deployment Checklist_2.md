---
source_file: "docker/config/openclaw/skills/i-ps/SKILL.md"
type: "document"
community: "Community 417"
location: "L20"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/Community_417
---

# Pre-Deployment Checklist

## Connections
- [[1. Change Documentation_2]] - `contains` [EXTRACTED]
- [[2. Testing Evidence_2]] - `contains` [EXTRACTED]
- [[3. Backups & Rollback_2]] - `contains` [EXTRACTED]
- [[4. Blast Radius_5]] - `contains` [EXTRACTED]
- [[5. Observability_2]] - `contains` [EXTRACTED]
- [[6. Security_2]] - `contains` [EXTRACTED]
- [[7. Communication_2]] - `contains` [EXTRACTED]
- [[8. Service Control Commands_2]] - `contains` [EXTRACTED]
- [[Skill Production Safety Checklist (PROD-SAFETY)_2]] - `contains` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/Community_417