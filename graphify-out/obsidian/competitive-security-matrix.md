---
source_file: "docs/security/competitive-security-matrix.md"
type: "document"
community: "Module Group 538"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/Module_Group_538
---

# competitive-security-matrix.md

## Connections
- [[AgentShroud Competitive Security Score 2626 (100%) vs nearest competitor 426 (15%)]] - `contains` [EXTRACTED]
- [[Unique Modules 12-26 (MCP proxy, web content scanner, DNS tunneling, etc. — no competitor has these)]] - `contains` [EXTRACTED]
- [[competitive-analysis-reports-archive.md (index of competitive reports)]] - `indexes` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/Module_Group_538