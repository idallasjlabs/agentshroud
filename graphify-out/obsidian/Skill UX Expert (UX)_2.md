---
source_file: "docker/config/openclaw/skills/i-ux/SKILL.md"
type: "document"
community: "Skill: UX Expert (UX)"
location: "L5"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/Skill_UX_Expert_UX
---

# Skill: UX Expert (UX)

## Connections
- [[Anti-Patterns to Flag_2]] - `contains` [EXTRACTED]
- [[CLI  TUI UX_2]] - `contains` [EXTRACTED]
- [[Core Discipline Discover → Define → Design → Validate_2]] - `contains` [EXTRACTED]
- [[Dashboard UX_2]] - `contains` [EXTRACTED]
- [[Dependencies_3]] - `contains` [EXTRACTED]
- [[Form UX_2]] - `contains` [EXTRACTED]
- [[Information Architecture_2]] - `contains` [EXTRACTED]
- [[Interaction Design Patterns_2]] - `contains` [EXTRACTED]
- [[Nielsen's 10 Heuristics — GSDE&G Application_2]] - `contains` [EXTRACTED]
- [[Role_21]] - `contains` [EXTRACTED]
- [[Rules_2]] - `contains` [EXTRACTED]
- [[Usability Audit Checklist_2]] - `contains` [EXTRACTED]
- [[User Flow Diagrams_2]] - `contains` [EXTRACTED]
- [[openclawskillsi-crSKILL]] - `contains` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/Skill_UX_Expert_UX