---
source_file: "firmware/voice-terminal/test/test_playback_state/test_playback_state.c"
type: "code"
community: "Playback State (test_playback_state)"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Playback_State_test_playback_state
---

# test_playback_state.c

## Connections
- [[String]] - `imports` [EXTRACTED]
- [[face_set_state()]] - `contains` [EXTRACTED]
- [[main()_8]] - `contains` [EXTRACTED]
- [[playback_step()]] - `contains` [EXTRACTED]
- [[reset_all()]] - `contains` [EXTRACTED]
- [[test_drain_clears_playing_and_returns_idle()]] - `contains` [EXTRACTED]
- [[test_drain_keeps_face_off_idle_when_retriggered()]] - `contains` [EXTRACTED]
- [[test_gate_open_sets_speaking_and_tts_playing()]] - `contains` [EXTRACTED]
- [[test_gate_opens_on_20s_age()]] - `contains` [EXTRACTED]
- [[test_gate_opens_on_768kb_cap()]] - `contains` [EXTRACTED]
- [[test_gate_opens_on_reply_complete()]] - `contains` [EXTRACTED]
- [[test_gate_stays_closed_before_any_cap()]] - `contains` [EXTRACTED]
- [[test_gate_stays_closed_leaves_state_idle()]] - `contains` [EXTRACTED]
- [[test_resume_offset_first_attempt_is_zero()]] - `contains` [EXTRACTED]
- [[test_resume_offset_rewinds_8kb()]] - `contains` [EXTRACTED]
- [[test_track_sent_ok_is_monotonic()]] - `contains` [EXTRACTED]
- [[vt_agent_count()_1]] - `contains` [EXTRACTED]
- [[vt_remote_log()_1]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Playback_State_test_playback_state