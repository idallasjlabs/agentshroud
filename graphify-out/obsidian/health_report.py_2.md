---
source_file: "docs/vault/02 - Modules/Security Modules/health_report.py.md"
type: "document"
community: "docs/vault"
location: "L10"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/docs/vault
---

# health_report.py

## Connections
- [[Configuration  Environment Variables_23]] - `contains` [EXTRACTED]
- [[Function Details_40]] - `contains` [EXTRACTED]
- [[Key Classes  Functions_43]] - `contains` [EXTRACTED]
- [[Purpose_124]] - `contains` [EXTRACTED]
- [[Related_47]] - `contains` [EXTRACTED]
- [[Responsibilities_45]] - `contains` [EXTRACTED]
- [[SQLite Health History DB]] - `uses` [EXTRACTED]
- [[Threat Model_16]] - `contains` [EXTRACTED]
- [[Tool Scoring Weights (trivy0.25, falco0.25, clamav0.20, wazuh0.15, gateway0.15)]] - `contains` [EXTRACTED]
- [[Weighted Composite Security Score (0-100, A-F grade)]] - `implements` [EXTRACTED]
- [[api.py (gatewaywebapi.py)]] - `references` [INFERRED]
- [[falco_monitor.py_2]] - `aggregates` [EXTRACTED]
- [[health_report.py_1]] - `contains` [EXTRACTED]
- [[proxy_status.py (gatewaydashboardproxy_status.py)]] - `references` [EXTRACTED]
- [[resource_guard.py_2]] - `references` [EXTRACTED]
- [[trivy_report.py_2]] - `feeds_into` [EXTRACTED]
- [[wazuh_client.py_2]] - `feeds_into` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/docs/vault