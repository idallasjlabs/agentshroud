---
source_file: "docs/vault/02 - Modules/Security Modules/session_security.py.md"
type: "document"
community: "skills/openclaw"
location: "L10"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/skills/openclaw
---

# session_security.py

## Connections
- [[Configuration  Environment Variables_28]] - `contains` [EXTRACTED]
- [[Event Injection Prevention (trusted source registry)]] - `implements` [EXTRACTED]
- [[Function Details_47]] - `contains` [EXTRACTED]
- [[Key Classes  Functions_50]] - `contains` [EXTRACTED]
- [[Purpose_131]] - `contains` [EXTRACTED]
- [[Related_54]] - `contains` [EXTRACTED]
- [[Responsibilities_52]] - `contains` [EXTRACTED]
- [[Session Hijacking Prevention (IP+UA fingerprint binding)]] - `implements` [EXTRACTED]
- [[SessionManager (Class)]] - `contains` [EXTRACTED]
- [[Threat Model_23]] - `contains` [EXTRACTED]
- [[log_sanitizer.py_2]] - `references` [EXTRACTED]
- [[oauth_security.py_2]] - `references` [EXTRACTED]
- [[session_security.py_1]] - `contains` [EXTRACTED]
- [[subagent_monitor.py_2]] - `references` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/skills/openclaw