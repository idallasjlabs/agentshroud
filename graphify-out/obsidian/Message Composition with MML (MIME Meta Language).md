---
source_file: "skills/openclaw/himalaya/references/message-composition.md"
type: "document"
community: "Community 452"
location: "L1"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/Community_452
---

# Message Composition with MML (MIME Meta Language)

## Connections
- [[Basic Message Structure]] - `contains` [EXTRACTED]
- [[Composing from CLI]] - `contains` [EXTRACTED]
- [[Headers]] - `contains` [EXTRACTED]
- [[MML Tag Reference]] - `contains` [EXTRACTED]
- [[MML for Rich Emails]] - `contains` [EXTRACTED]
- [[Plain Text Body]] - `contains` [EXTRACTED]
- [[Tips_3]] - `contains` [EXTRACTED]
- [[message-composition]] - `contains` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/Community_452