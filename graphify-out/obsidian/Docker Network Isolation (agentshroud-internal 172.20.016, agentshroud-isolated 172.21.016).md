---
source_file: "docker/README.md"
type: "concept"
community: "Module Group 138"
location: "Architecture section"
tags:
  - graphify/concept
  - graphify/EXTRACTED
  - community/Module_Group_138
---

# Docker Network Isolation (agentshroud-internal 172.20.0/16, agentshroud-isolated 172.21.0/16)

## Connections
- [[AgentShroud Security Hardening Plan]] - `describes` [EXTRACTED]
- [[Container Stack Architecture (gateway + openclaw + hermes + hci)]] - `conceptually_related_to` [EXTRACTED]
- [[Docker Compose Production (docker-compose.yml)]] - `implements` [EXTRACTED]
- [[Docker VPN Networking Fix (Cisco AnyConnect + vpnkit-userspace)]] - `conceptually_related_to` [INFERRED]
- [[Falco Runtime Security (eBPF kernel monitoring)]] - `complements` [INFERRED]
- [[Security Hardening Phases P0–FINAL]] - `culminates_in` [EXTRACTED]

#graphify/concept #graphify/EXTRACTED #community/Module_Group_138
