---
source_file: "docker/config/openclaw/skills/i-aws/SKILL.md"
type: "document"
community: "Gateway Test Suite"
location: "L151"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/Gateway_Test_Suite
---

# FY26 Cost Reduction Context

## Connections
- [[AWS Cloud Management & FinOps Agent]] - `contains` [EXTRACTED]
- [[AWS Cloud Management & FinOps Agent_1]] - `contains` [EXTRACTED]
- [[In-Scope Departments (Cost Reduction)_1]] - `contains` [EXTRACTED]
- [[Infrastructure You Must Know_1]] - `contains` [EXTRACTED]
- [[Out-of-Scope Departments (Inventory & Tag Only)_1]] - `contains` [EXTRACTED]
- [[Savings Levers (Ranked by Impact)_1]] - `contains` [EXTRACTED]
- [[Target 40% Full-Year Reduction on Global Services Resources_1]] - `contains` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/Gateway_Test_Suite