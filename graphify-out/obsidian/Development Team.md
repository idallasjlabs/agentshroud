---
source_file: "docs/TEAM.md"
type: "document"
community: "Team (docs)"
location: "L18"
tags:
  - graphify/document
  - graphify/EXTRACTED
  - community/Team_docs
---

# Development Team

## Connections
- [[AgentShroud Development Team — Agile Structure]] - `contains` [EXTRACTED]
- [[Data Engineers  Analysts]] - `contains` [EXTRACTED]
- [[DevOps  Platform Engineer — CICD, Infrastructure, Reliability]] - `contains` [EXTRACTED]
- [[Engineers (Backend, Frontend, Full-Stack)]] - `contains` [EXTRACTED]
- [[QA  Test Engineers — Embedded, Not a Separate Gate]] - `contains` [EXTRACTED]
- [[UX  Design — One Sprint Ahead]] - `contains` [EXTRACTED]

#graphify/document #graphify/EXTRACTED #community/Team_docs