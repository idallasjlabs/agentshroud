---
source_file: "gateway/tests/test_soc_realtime_coverage.py"
type: "code"
community: "SOC Authentication"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/SOC_Authentication
---

# test_soc_realtime_coverage.py

## Connections
- [[Action_1]] - `imports` [EXTRACTED]
- [[PermissionResult]] - `imports` [EXTRACTED]
- [[RBACConfig_1]] - `imports` [EXTRACTED]
- [[Resource_1]] - `imports` [EXTRACTED]
- [[Role_1]] - `imports` [EXTRACTED]
- [[SCLCaller]] - `imports` [EXTRACTED]
- [[SOCWebSocketHandler]] - `imports` [EXTRACTED]
- [[SecurityEvent_1]] - `imports` [EXTRACTED]
- [[Severity_1]] - `imports` [EXTRACTED]
- [[TestCoerceToWSEventExtra]] - `contains` [EXTRACTED]
- [[TestCollectRecentEvents]] - `contains` [EXTRACTED]
- [[TestEventFanOut]] - `contains` [EXTRACTED]
- [[TestFromAnomalyAlert]] - `contains` [EXTRACTED]
- [[TestFromAuditChainEntry]] - `contains` [EXTRACTED]
- [[TestFromDict]] - `contains` [EXTRACTED]
- [[TestFromEgressAttempt]] - `contains` [EXTRACTED]
- [[TestFromPipelineResult]] - `contains` [EXTRACTED]
- [[TestGetConfigToken]] - `contains` [EXTRACTED]
- [[TestHandlerRun]] - `contains` [EXTRACTED]
- [[TestMapSeverity]] - `contains` [EXTRACTED]
- [[TestResolveCaller]] - `contains` [EXTRACTED]
- [[TestSCLCaller]] - `contains` [EXTRACTED]
- [[TestSendEventAndKeepalive]] - `contains` [EXTRACTED]
- [[TestTokenStorePruning]] - `contains` [EXTRACTED]
- [[TestVerifyBearer]] - `contains` [EXTRACTED]
- [[TestWSSOCEndpoint]] - `contains` [EXTRACTED]
- [[WSEvent]] - `imports` [EXTRACTED]
- [[WSEventType]] - `imports` [EXTRACTED]
- [[_FakeBus]] - `contains` [EXTRACTED]
- [[_FakeRBAC_1]] - `contains` [EXTRACTED]
- [[_audit_entry()]] - `contains` [EXTRACTED]
- [[_block_run_secrets()]] - `contains` [EXTRACTED]
- [[_coerce_to_ws_event()]] - `imports` [EXTRACTED]
- [[_get_config_token()]] - `imports` [EXTRACTED]
- [[_get_rbac_manager()]] - `imports` [EXTRACTED]
- [[_make_ws()]] - `contains` [EXTRACTED]
- [[_resolve_caller()]] - `imports` [EXTRACTED]
- [[_spin()]] - `contains` [EXTRACTED]
- [[_verify_bearer()]] - `imports` [EXTRACTED]
- [[_verify_session_token()]] - `imports` [EXTRACTED]
- [[clean_auth_env()]] - `contains` [EXTRACTED]
- [[event_adapter.py]] - `imports_from` [EXTRACTED]
- [[get_caller()]] - `imports` [EXTRACTED]
- [[issue_session_token()]] - `imports` [EXTRACTED]
- [[issue_ws_token()]] - `imports` [EXTRACTED]
- [[redeem_ws_token()]] - `imports` [EXTRACTED]
- [[ws_soc_endpoint()]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/SOC_Authentication