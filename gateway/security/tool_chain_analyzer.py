...
_PARAM_INJECTION_PATTERNS: list[tuple[re.Pattern, str]] = [
    ...
    (
        re.compile(
            r"(?:'\s*OR\s*'?1'?\s*=\s*'?1|UNION\s+(?:ALL\s+)?SELECT|DROP\s+TABLE|INSERT\s+INTO\s+\w+\s+VALUES)",
            re.IGNORECASE,
        ),
        "sql_injection",
    ),
...
class ToolChainAnalyzer:
        ...
        self.patterns: List[ChainPattern] = [
            ...
            ChainPattern(
                ...
                source_pattern=re.compile(
                    r"\bread.*(/run/secrets|/etc/passwd|credential|secret|key)", re.IGNORECASE
                ),
            ...
            ChainPattern(
                ...
                source_pattern=re.compile(
                    r"\bread.*(config|env|\.yaml|\.json|\.conf)", re.IGNORECASE
                ),
    ...
    def analyze_tool_call(
        self, session_id: str, tool_name: str, parameters: Dict[str, Any], call_id: str = ""
        ...
        if chain_match:
            ...
            if chain_match.pattern.action == ChainAction.BLOCK:
                ...
                logger.error(
                    f"BLOCKING tool call {tool_name} in session {session_id} due to pattern: {chain_match.pattern.name}"
                )
                ...
                logger.warning(
                    f"APPROVAL REQUIRED for tool call {tool_name} in session {session_id} due to pattern: {chain_match.pattern.name}"
                )
                ...
                logger.warning(
                    f"Suspicious tool call pattern detected: {chain_match.pattern.name} in session {session_id}"
                )

        return True, chain_match

    def _detect_chain_patterns(
        self, session: SessionChainContext, current_call: ToolCall
    ) -> Optional[ChainMatch]:
    ...
    def _matches_source_pattern(self, pattern: ChainPattern, call: ToolCall) -> bool:
        ...
        if "credential" in pattern.name:
            ...
            if file_path and (
                "/run/secrets" in file_path
                or "/etc/passwd" in file_path
                or "secret" in file_path.lower()
                or "credential" in file_path.lower()
                or "key" in file_path.lower()
            ):
                return True
            return False
        ...
        if "config" in pattern.name:
            file_path = call.parameters.get("file_path", "")
            if file_path and (
                file_path.endswith((".yaml", ".json", ".conf", ".cfg"))
    ...
    def _calculate_risk_score(
        self, pattern: ChainPattern, source_calls: List[ToolCall], sink_calls: List[ToolCall]
    ) -> float:
    ...
    def get_session_stats(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            - (session.tool_calls[0].timestamp if session.tool_calls else time.time()),
        }
