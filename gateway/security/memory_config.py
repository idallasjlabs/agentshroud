...
class MemoryIntegrityConfig:
    ...
    protected_files: Set[str] = field(
        default_factory=lambda: {"SOUL.md", "AGENTS.md", "BRAND.md", "USER.md"}
    )

    # Memory files to monitor for integrity
    monitored_files: Set[str] = field(
        default_factory=lambda: {"MEMORY.md", "HEARTBEAT.md", "TOOLS.md"}
    )

    # Directories to monitor for daily notes
    monitored_directories: Set[str] = field(default_factory=lambda: {"memory"})
...
class MemorySecurityConfig:
    ...
    def from_env(cls) -> "MemorySecurityConfig":
        ...
        config.integrity.enabled = (
            os.getenv("AGENTSHROUD_MEMORY_INTEGRITY_ENABLED", "true").lower() == "true"
        )
        config.lifecycle.enabled = (
            os.getenv("AGENTSHROUD_MEMORY_LIFECYCLE_ENABLED", "true").lower() == "true"
        )
        config.lifecycle.pii_scanning_enabled = (
            os.getenv("AGENTSHROUD_MEMORY_PII_SCAN_ENABLED", "true").lower() == "true"
        )
        config.lifecycle.injection_scanning_enabled = (
            os.getenv("AGENTSHROUD_MEMORY_INJECTION_SCAN_ENABLED", "true").lower() == "true"
        )
