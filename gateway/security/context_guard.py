...
class ContextGuard:
    ...
    def analyze_message(self, session_id: str, message: str) -> List[ContextAttack]:
        ...
        instruction_attacks = self._detect_instruction_injection(session_id, message, current_time, context_size_before)
        attacks.extend(instruction_attacks)
        
        # Check for repetition attacks
        repetition_attacks = self._detect_repetition_attacks(session_id, message, current_time, context_size_before)
        attacks.extend(repetition_attacks)
        
        # Check for rapid context growth
        rapid_growth_attacks = self._detect_rapid_context_growth(session, message, current_time, context_size_before)
        attacks.extend(rapid_growth_attacks)
        
        # Check for hidden instructions
        hidden_attacks = self._detect_hidden_instructions(session_id, message, current_time, context_size_before)
    ...
    def _detect_instruction_injection(self, session_id: str, message: str, timestamp: float, context_size_before: int) -> List[ContextAttack]:
    ...
    def _detect_repetition_attacks(self, session_id: str, message: str, timestamp: float, context_size_before: int) -> List[ContextAttack]:
        ...
        for i, pattern in enumerate(self.repetition_patterns):
            ...
            if matches:
                ...
                total_repeated_chars = sum(len(match) * (message.count(match) - 1) for match in matches)
        ...
        session = self.sessions[session_id]
        session.repeated_content_hashes[content_hash] = session.repeated_content_hashes.get(content_hash, 0) + 1
    ...
    def _detect_rapid_context_growth(self, session: SessionContext, message: str, timestamp: float, context_size_before: int) -> List[ContextAttack]:
    ...
    def _detect_hidden_instructions(self, session_id: str, message: str, timestamp: float, context_size_before: int) -> List[ContextAttack]:
        ...
        if len(lines) > 50:  # Large message
            ...
            for line in lines:
                ...
                if any(keyword in line_stripped for keyword in ['ignore', 'disregard', 'system', 'override', 'act as', 'pretend']):
            ...
            if instruction_like_lines:
                ...
                if ratio < 0.1:  # Less than 10% - might be dilution attack
                    ...
                    attacks.append(ContextAttack(
                        ...
                        message_content='; '.join(instruction_like_lines[:3]) + ("..." if len(instruction_like_lines) > 3 else ""),
                        detection_patterns=detected_patterns,
                        timestamp=timestamp,
                        context_size_before=context_size_before,
                        context_size_after=context_size_before + len(message)
    ...
    def get_session_risk_level(self, session_id: str) -> str:
        ...
        session_attacks = [attack for attack in self.detected_attacks if attack.session_id == session_id]
    ...
    def should_block_message(self, session_id: str, message: str) -> Tuple[bool, List[str]]:
        ...
        for attack in attacks:
            ...
            elif attack.severity == "high" and attack.attack_type in ["message_size_limit", "context_size_limit"]:
    ...
    def get_attack_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'high_risk_sessions': [
                session_id for session_id in self.sessions.keys()
...
def check_message(text: str) -> tuple[bool, list[str]]:
    ...
    for attack in attacks:
        if attack.attack_type == "repetition":
            findings.append(f"Repeated pattern detected: {attack.details.get('pattern_preview', 'Unknown pattern')}")
            allowed = False
    ...
    if total_chars > 1000:  # Only check for longer messages
        ...
        if entropy_ratio < 0.1:  # Very low entropy suggests repetition
            findings.append(f"Low entropy detected: {entropy_ratio:.3f} (possible instruction dilution)")
            allowed = False
