...
class MemoryLifecycleManager:
    ...
    def scan_content_for_threats(self, content: str, file_path: str) -> List[ContentThreat]:
        ...
        # Size check
        if len(content.encode("utf-8")) > self.config.memory_md_max_size and file_path.endswith(
            "MEMORY.md"
        ):
            threat = ContentThreat(
            ...
            if threat.threat_type == ContentThreatType.PII_DETECTED:
                ...
                if threat.matched_pattern:
                    sanitized_content = sanitized_content.replace(
                        threat.matched_pattern, "[REMOVED-INJECTION-ATTEMPT]"
                    )
    ...
    def validate_memory_write(
        ...
        for threat in threats:
            ...
            if threat.severity == "CRITICAL":
                logger.critical(
                    f"SECURITY THREAT: {threat.threat_type.value} in {path_str}: {threat.details}"
                )
            elif threat.severity == "HIGH":
                logger.error(
                    f"HIGH RISK: {threat.threat_type.value} in {path_str}: {threat.details}"
                )
            else:
    ...
    def enforce_memory_md_size_limit(self):
        ...
        try:
            ...
            if current_size > self.config.memory_md_max_size:
                logger.warning(
                    f"MEMORY.md exceeds size limit: {current_size} > {self.config.memory_md_max_size}"
                )

                # Archive current version
                archive_path = self.archive_file(
                    memory_md_path, f"size_limit_exceeded_{current_size}bytes"
                )
                ...
                if archive_path:
                    ...
                    logger.info(
                        f"Truncated MEMORY.md from {current_size} to {action.final_size} bytes"
                    )
    ...
    def _cleanup_old_threats(self, days: int = 30):
        ...
        self.detected_threats = [
            threat for threat in self.detected_threats if threat.timestamp > cutoff_time
        ]
        if len(self.detected_threats) < original_count:
            logger.debug(
                f"Cleaned up {original_count - len(self.detected_threats)} old threat records"
            )
    ...
    def _cleanup_old_actions(self, days: int = 30):
        ...
        self.retention_actions = [
            action for action in self.retention_actions if action.timestamp > cutoff_time
        ]
        if len(self.retention_actions) < original_count:
            logger.debug(
                f"Cleaned up {original_count - len(self.retention_actions)} old retention action records"
            )
