...
class MemoryIntegrityMonitor:
            ...
            if self.write_window_db_path.exists():
                with open(self.write_window_db_path, "r") as f:
                    ...
                    self.active_write_windows = {
                        path: end_time for path, end_time in data.items() if end_time > current_time
                    }
        except Exception as e:
    ...
    def register_expected_write(
        self, file_path: str, source: ModificationSource = ModificationSource.AGENT
    ):
            ...
            if existing_record:
                ...
                if existing_record.hash_sha256 != current_hash:
                    ...
                    if is_protected and source != ModificationSource.HUMAN:
                        ...
                        logger.critical(
                            f"SECURITY ALERT: Protected file {path_str} was modified by {source.value}"
                        )
    ...
    def clear_old_alerts(self, days: int = 7):
        ...
        self.modification_alerts = [
            alert for alert in self.modification_alerts if alert["timestamp"] > cutoff_time
        ]
