...
class LogSanitizer(logging.Filter):
    ...
    def _compile_patterns(self) -> Dict[str, Pattern[str]]:
        ...
        return {
            ...
            'password_assignment': re.compile(r'password\s*[:=]\s*["\']?[^"\'\s]{3,}["\']?', re.IGNORECASE),
            'token_assignment': re.compile(r'token\s*[:=]\s*["\']?[^"\'\s]{3,}["\']?', re.IGNORECASE),
            'key_assignment': re.compile(r'[^_]key\s*[:=]\s*["\']?[^"\'\s]{3,}["\']?', re.IGNORECASE),
            'secret_assignment': re.compile(r'secret\s*[:=]\s*["\']?[^"\'\s]{3,}["\']?', re.IGNORECASE),
    ...
    def _sanitize_text(self, text: str) -> str:
        ...
        for pattern_name, pattern in self.patterns.items():
            if pattern_name == 'ssn':
                ...
                sanitized = pattern.sub(lambda m: m.group(0).split(':')[0] + ':[REDACTED]' if ':' in m.group(0) else m.group(0).split('=')[0] + '=[REDACTED]', sanitized)
