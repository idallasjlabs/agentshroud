...
from .key_rotation_config import KeyRotationConfig, CredentialRotationPolicy
...
class CredentialInfo:
    ...
    def is_in_grace_period(self) -> bool:
        ...
        return (
            self.grace_period_end is not None and
            datetime.now(timezone.utc) < self.grace_period_end
        )
...
class KeyRotationManager:
    ...
    def get_credential_status(self, credential_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "grace_period_end": cred.grace_period_end.isoformat() if cred.grace_period_end else None,
    ...
    def get_all_credentials_status(self) -> List[Dict[str, Any]]:
        ...
        return [
            status for cred_id in self._credentials
    ...
    async def rotate_credential(
        ...
        if cred.status == RotationStatus.IN_PROGRESS:
            return {
                "success": False,
                "error": f"Rotation already in progress for {credential_id}"
            }
        ...
        async with self._rotation_lock:
            try:
                ...
                asyncio.create_task(
                    self._retire_old_credential_after_grace_period(credential_id)
                )
    ...
    async def emergency_rotate_credential(
        ...
        if not self.config.is_emergency_trigger_enabled(trigger_reason):
            return {
                "success": False,
                "error": f"Emergency trigger {trigger_reason} is not enabled"
            }
    ...
    def cleanup_retired_credentials(self) -> Dict[str, Any]:
        ...
        cleaned = []
        for cred_id, cred in self._credentials.items():
            if (cred.grace_period_end and 
                datetime.now(timezone.utc) > cred.grace_period_end and
                cred.old_op_reference):
