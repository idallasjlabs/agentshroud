...
class ApprovalHardeningConfig:
    ...
    high_risk_keywords: List[str] = field(default_factory=lambda: [
        "rm", "delete", "remove", "drop", "truncate", "format", 
        "sudo", "chmod", "chown", "kill", "pkill", "killall",
        "init", "shutdown", "reboot", "halt", "poweroff",
        "mv", "move", "rename", "cp", "copy", "dd",
        "curl", "wget", "ssh", "scp", "ftp", "sftp",
        "exec", "eval", "system", "shell", "bash", "sh"
    ])
...
class ApprovalHardening:
    ...
    def analyze_request(
        ...
        self._check_description_parameter_mismatch(
            tool_name, description, parameters, detection
        )
        ...
        self._check_repeat_request_patterns(
            tool_name, parameters, agent_id, detection
        )
    ...
    def _normalize_description(self, description: str) -> str:
        ...
        misleading_phrases = [
            "just", "simply", "only", "harmless", "safe", "routine",
            "standard", "normal", "typical", "regular", "basic",
            "innocent", "benign", "minor", "quick", "small"
        ]
    ...
    def _check_description_parameter_mismatch(
        ...
        has_dangerous_params = any(
            keyword in param_text for keyword in dangerous_commands
        )
        
        has_benign_description = any(
            word in desc_lower for word in [
                "check", "list", "view", "show", "display", "get", 
                "read", "status", "info", "help", "test"
            ]
        )
        ...
        if tool_name == "exec":
            ...
            if isinstance(command, str):
                ...
                if any(danger in cmd_lower for danger in ["rm -rf", "format", "del /q"]):
                    if not any(warn in desc_lower for warn in ["delete", "remove", "format", "destroy"]):
                        detection.issues.append(
                            "Destructive command execution not clearly indicated in description"
                        )
    
    def _check_misleading_language(
        self, description: str, detection: DeceptionDetection
    ) -> None:
        ...
        for pattern in authority_patterns:
            if re.search(pattern, desc_lower):
                detection.issues.append(
                    "Description contains authority or legitimacy claims"
                )
                break
    ...
    def _check_parameter_obfuscation(
        ...
        if re.search(base64_pattern, param_str):
            detection.issues.append(
                "Parameters may contain base64-encoded content"
            )
        ...
        if re.search(hex_pattern, param_str):
            detection.issues.append(
                "Parameters may contain hex-encoded content"
            )
        ...
        if re.search(url_pattern, param_str):
            detection.issues.append(
                "Parameters may contain URL-encoded content"
            )
    ...
    def _check_repeat_request_patterns(
        ...
        if fingerprint in self._parameter_fingerprints[agent_id]:
            detection.issues.append(
                "Similar request parameters have been submitted recently"
            )
    ...
    def _create_parameter_fingerprint(
    ...
    def _cleanup_old_denied_requests(self, agent_id: str) -> None:
        ...
        self._denied_requests[agent_id] = [
            req for req in self._denied_requests[agent_id]
    ...
    def format_hardened_message(
        ...
        if detection.risk_score > 0:
            risk_level = "HIGH" if detection.risk_score >= 0.75 else "MEDIUM" if detection.risk_score >= 0.5 else "LOW"
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        active_cooldowns = len([
            agent_id for agent_id, requests in self._denied_requests.items()
            if requests
        ])
