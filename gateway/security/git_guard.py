...
class GitGuard:
    ...
    def __init__(self, mode: str = "monitor"):
        ...
        self.malicious_patterns = {
            ...
            'reverse_shell': (ThreatLevel.CRITICAL, [
            ...
            'outbound_requests': (ThreatLevel.HIGH, [
                r'curl\s+.*https?://',
                r'wget\s+.*https?://',
                r'fetch\s+.*https?://',
                r'http\.get\(',
            ...
            'encoded_payloads': (ThreatLevel.CRITICAL, [
                r'base64\s+-d.*exec',
            ...
            'file_manipulation': (ThreatLevel.MEDIUM, [
        ...
        self.package_patterns = {
            'postinstall_risks': (ThreatLevel.HIGH, [
            ...
            'preinstall_risks': (ThreatLevel.HIGH, [
                r'"preinstall":\s*"[^"]*curl',
                r'"preinstall":\s*"[^"]*wget',
        ...
        try:
            ...
            try:
                ...
                for script_name, script_content in scripts.items():
                    if script_name in ['postinstall', 'preinstall', 'prepare', 'install']:
                        findings.extend(self._analyze_script_content(
                            str(package_file), 
                            script_content,
    ...
    def _analyze_file_content(self, file_path: Path, content: str, category: str) -> List[SecurityFinding]:
        """Analyze file content for malicious patterns."""
        findings = []
        lines = content.split('\n')
        
        for pattern_category, (threat_level, patterns) in self.malicious_patterns.items():
        ...
        if file_path.name == 'package.json':
            for pattern_category, (threat_level, patterns) in self.package_patterns.items():
                for pattern in patterns:
                    regex = re.compile(pattern, re.IGNORECASE | re.MULTILINE)
    ...
    def _analyze_script_content(self, file_path: str, script_content: str, category: str) -> List[SecurityFinding]:
            ...
            if finding.threat_level in [ThreatLevel.CRITICAL, ThreatLevel.HIGH]:
                ...
                if file_path.exists() and str(file_path) not in quarantined_files:
                    try:
                        ...
                        logger.warning(f"Quarantined suspicious file: {file_path} -> {quarantine_path}")
