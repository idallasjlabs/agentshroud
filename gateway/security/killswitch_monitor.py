...
class KillSwitchMonitor:
    ...
    def verify_killswitch(self, dry_run: Optional[bool] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if (result["overall_status"] in ["FAIL", "ERROR"] and 
                self.config.alert_on_verification_failure and 
    ...
    def heartbeat_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if response_time <= self.config.heartbeat_timeout.total_seconds():
                ...
                result["status"] = "SLOW"
                result["details"].append(f"Response time {response_time:.2f}s exceeds timeout {self.config.heartbeat_timeout.total_seconds()}s")
        ...
        if (self._consecutive_heartbeat_misses >= self.config.heartbeat_miss_threshold and
            self.config.alert_on_heartbeat_miss and 
            self.alert_dispatcher):
    ...
    def anomaly_detection(
        ...
        try:
            ...
            result["metrics"] = {
                ...
                "tokens_last_hour": sum(tokens for ts, tokens in self._token_usage_history if now - ts <= 3600),
        ...
        if (result["overall_status"] == "ANOMALY_DETECTED" and
            self.config.alert_on_anomaly and 
            self.alert_dispatcher):
    ...
    def _test_script_syntax(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result = subprocess.run(["bash", "-n", str(self.config.killswitch_script_path)], 
                                  capture_output=True, 
                                  text=True, 
                                  timeout=5)
            return {
