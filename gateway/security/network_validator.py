...
class NetworkValidator:
    ...
    def validate_docker_compose_config(self, compose_file_path: str) -> List[NetworkSecurityFinding]:
        """
    ...
    def _parse_service_network_config(self, service_name: str, service_config: Dict[str, Any]) -> NetworkConfiguration:
    ...
    def _validate_service_network_isolation(self, service_configs: Dict[str, NetworkConfiguration]) -> List[NetworkSecurityFinding]:
        """Validate service network isolation."""
        findings = []
        
        for service_name, config in service_configs.items():
            # Check if OpenClaw container is on internal network only
            if service_name.startswith('openclaw') and service_name != self.gateway_service:
    ...
    def _validate_port_exposure(self, service_configs: Dict[str, NetworkConfiguration]) -> List[NetworkSecurityFinding]:
                ...
                if ':' in port_mapping:
                    ...
                    if host_port in dangerous_ports:
                        findings.append(NetworkSecurityFinding(
                            ...
                            severity="high" if host_port in ['22', '3389', '5432', '3306'] else "medium",
                            service_name=service_name,
                            description=f"Dangerous port {host_port} ({dangerous_ports[host_port]}) exposed",
                            details={"port_mapping": port_mapping, "service": dangerous_ports[host_port]},
    ...
    def _validate_dns_configuration(self, service_configs: Dict[str, NetworkConfiguration]) -> List[NetworkSecurityFinding]:
                ...
                for dns_server in config.dns_servers:
                    if dns_server in public_dns_servers:
                        findings.append(NetworkSecurityFinding(
                            ...
                            details={"dns_servers": config.dns_servers, "public_server": dns_server},
                            remediation="Configure DNS to point to gateway for filtering"
                        ))
        
        return findings
    
    def _validate_network_modes(self, service_configs: Dict[str, NetworkConfiguration]) -> List[NetworkSecurityFinding]:
    ...
    def _validate_container_runtime_config(self, container) -> List[NetworkSecurityFinding]:
        ...
        try:
            ...
            if exposed_ports and container_name.startswith('openclaw') and 'gateway' not in container_name:
                findings.append(NetworkSecurityFinding(
