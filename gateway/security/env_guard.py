...
class EnvironmentGuard:
    ...
    def __init__(self):
        ...
        self.api_key_patterns = [
            ...
            re.compile(r'\bxoxb-[A-Za-z0-9]{11}-[A-Za-z0-9]{11}-[A-Za-z0-9]{24}\b'),  # Slack Bot Token
            re.compile(r'\bxoxp-[A-Za-z0-9]{11}-[A-Za-z0-9]{11}-[A-Za-z0-9]{11}-[A-Za-z0-9]{32}\b'),  # Slack User Token
        ...
        self.credential_env_vars = {
            ...
            'PASSWORD', 'PASSWD', 'PWD', 'TOKEN', 'BEARER_TOKEN',
            'OPENAI_API_KEY', 'ANTHROPIC_API_KEY', 'GOOGLE_API_KEY',
            'AWS_ACCESS_KEY_ID', 'AWS_SECRET_ACCESS_KEY', 'AWS_SESSION_TOKEN',
            'GITHUB_TOKEN', 'GH_TOKEN', 'GITHUB_PAT',
            'SLACK_TOKEN', 'SLACK_WEBHOOK_URL',
            'OP_SERVICE_ACCOUNT_TOKEN', 'OP_SESSION',
        ...
        for blocked_path in self.blocked_paths:
            if '*' in blocked_path:
                ...
                if re.match(pattern, normalized_path):
                    logger.warning(f"Agent {agent_id} blocked from accessing {file_path} - environment leakage risk")
    ...
    def check_command_execution(self, command: str, agent_id: str) -> bool:
        ...
        try:
            ...
            if base_cmd in self.blocked_commands:
                logger.warning(f"Agent {agent_id} blocked from executing '{command}' - environment leakage risk")
                self._record_leakage(
            ...
            if self._contains_env_access_patterns(command):
                logger.warning(f"Agent {agent_id} blocked from executing '{command}' - potential environment access")
                self._record_leakage(
                    source=f"command:{command}",
                    leaked_vars=[],
                    severity="medium",
    ...
    def scrub_command_output(self, output: str, command: str) -> str:
        ...
        if leaked_vars:
            self._record_leakage(
                ...
                severity="high" if any(var in self.credential_env_vars for var in leaked_vars) else "medium",
    ...
    def monitor_environment_access(self, agent_id: str) -> Dict[str, Any]:
        ...
        agent_leakages = [
            leakage for leakage in self.detected_leakages
            if agent_id in leakage.context
        ]
        
        if agent_leakages:
            monitoring_results['environment_access_attempts'] = len(agent_leakages)
            monitoring_results['blocked_attempts'] = len([
                l for l in agent_leakages 
                if l.detection_method in ['path_block', 'command_block', 'pattern_block']
            ])
            
            # Determine risk level
            critical_attempts = len([l for l in agent_leakages if l.severity == 'critical'])
            high_attempts = len([l for l in agent_leakages if l.severity == 'high'])
    ...
    def _record_leakage(self, source: str, leaked_vars: List[str], severity: str, 
                       detection_method: str, context: str):
    ...
    def get_leakage_summary(self) -> Dict[str, Any]:
        ...
        for leakage in self.detected_leakages:
            # Count by severity
            summary['by_severity'][leakage.severity] = summary['by_severity'].get(leakage.severity, 0) + 1
    ...
    def export_leakage_report(self, output_path: str):
        ...
        report = {
            ...
            'leakages': [
                {
                    'source': l.source,
                    'leaked_vars': l.leaked_vars,
                    'severity': l.severity,
                    'detection_method': l.detection_method,
                    'context': l.context
                }
                for l in self.detected_leakages
            ]
        }
...
def check_command(cmd: str) -> tuple[bool, str]:
    ...
    if allowed:
        ...
        if any(pattern in cmd.lower() for pattern in ['/proc/self/environ', '/proc/', 'printenv', 'env |']):
