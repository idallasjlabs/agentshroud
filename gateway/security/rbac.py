...
class RBACManager:
    ...
    def check_permission(
        ...
        try:
            ...
            if (
                role == Role.ADMIN
                and action == Action.TOOL_USE
                and context
                and context.get("tool_tier") == ToolTier.CRITICAL
            ):
                return PermissionResult(
                    allowed=False,
            ...
            return PermissionResult(allowed=False, reason=f"Permission check error: {str(e)}")
    ...
    def check_tool_permission(self, user_id: str, tool_tier: ToolTier) -> PermissionResult:
        ...
        try:
            ...
            return PermissionResult(allowed=False, reason=f"Tool permission check error: {str(e)}")
    ...
    def set_user_role(
        self, admin_user_id: str, target_user_id: str, role: Role
    ) -> PermissionResult:
        ...
        try:
            ...
            if not self.config.is_owner(admin_user_id):
                return PermissionResult(
                    allowed=False, reason="Only the owner can modify user roles"
                )

            # Don't allow changing owner role
            if self.config.is_owner(target_user_id):
                return PermissionResult(allowed=False, reason="Cannot change owner role")
            ...
            return PermissionResult(allowed=False, reason=f"Role setting error: {str(e)}")
    ...
    def list_users_and_roles(self, requesting_user_id: str) -> PermissionResult:
        ...
        try:
            ...
            return PermissionResult(allowed=False, reason=f"Error listing users: {str(e)}")
