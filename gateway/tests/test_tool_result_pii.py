...
from gateway.ingest_api.models import RedactionResult, RedactionDetail
...
class TestToolResultSanitizer:
    ...
    async def test_sanitize_disabled(self, default_config):
        ...
        disabled_config = ToolResultPIIConfig(
            enabled=False,
            default_config=default_config
        )
    ...
    async def test_tool_specific_configuration(self, sanitizer):
        ...
        with patch('gateway.security.tool_result_sanitizer.PIISanitizer') as mock_sanitizer_class:
            mock_sanitizer = AsyncMock()
            mock_sanitizer.sanitize.return_value = RedactionResult(
                sanitized_content=test_content,
                redactions=[],
                entity_types_found=[]
            )
            mock_sanitizer_class.return_value = mock_sanitizer
...
class TestMiddlewareIntegration:
    ...
    async def test_process_tool_result_success(self, mock_config):
        ...
        with patch.object(middleware.tool_result_sanitizer, 'sanitize_tool_result') as mock_sanitize:
...
class TestRealWorldScenarios:
    ...
    async def test_icloud_contact_scanning(self):
        ...
        with patch('gateway.security.tool_result_sanitizer.PIISanitizer') as mock_sanitizer_class:
            ...
            mock_sanitizer.sanitize.return_value = RedactionResult(
                sanitized_content="John Doe\n<PHONE_NUMBER>\n<EMAIL_ADDRESS>\nJane Smith\n<PHONE_NUMBER>\n<EMAIL_ADDRESS>\n2",
                redactions=[
                    RedactionDetail(entity_type="PHONE_NUMBER", start=9, end=21, score=0.9, replacement="<PHONE_NUMBER>"),
                    RedactionDetail(entity_type="EMAIL_ADDRESS", start=22, end=43, score=0.95, replacement="<EMAIL_ADDRESS>"),
                    RedactionDetail(entity_type="PHONE_NUMBER", start=55, end=67, score=0.9, replacement="<PHONE_NUMBER>"),
                    RedactionDetail(entity_type="EMAIL_ADDRESS", start=68, end=93, score=0.95, replacement="<EMAIL_ADDRESS>")
                ],
    ...
    async def test_email_content_scanning(self):
        ...
        with patch('gateway.security.tool_result_sanitizer.PIISanitizer') as mock_sanitizer_class:
            mock_sanitizer = AsyncMock()
            mock_sanitizer.sanitize.return_value = RedactionResult(
                sanitized_content=email_content.replace("123-45-6789", "<US_SSN>").replace("4111-1111-1111-1111", "<CREDIT_CARD>").replace("support@bank.com", "<EMAIL_ADDRESS>"),
                redactions=[
                    RedactionDetail(entity_type="US_SSN", start=0, end=11, score=0.95, replacement="<US_SSN>"),
                    RedactionDetail(entity_type="CREDIT_CARD", start=0, end=19, score=0.9, replacement="<CREDIT_CARD>"),
                    RedactionDetail(entity_type="EMAIL_ADDRESS", start=0, end=16, score=0.98, replacement="<EMAIL_ADDRESS>")
                ],
