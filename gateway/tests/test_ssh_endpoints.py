...
class TestSSHDisabledEndpoint:
    ...
    def test_ssh_exec_disabled_returns_503(self, disabled_client, auth_headers):
        resp = disabled_client.post("/ssh/exec", json={"host": "pi", "command": "ls"}, headers=auth_headers)
...
class TestSSHRequireApprovalFalse:
    ...
    async def no_approval_client(self):
        ...
        cfg = GatewayConfig(
            ...
            auth_method="shared_secret", auth_token="test-token-12345",
            ledger=LedgerConfig(backend="sqlite", path=tmp_path, retention_days=90),
            router=RouterConfig(enabled=True, default_target="test-agent", targets={}),
            pii=PIIConfig(engine="regex", entities=[], enabled=True),
            approval_queue=ApprovalQueueConfig(enabled=True, actions=[], timeout_seconds=3600),
            log_level="WARNING", ssh=ssh_cfg,
        )
    ...
    def test_non_auto_approved_executes_directly(self, no_approval_client, auth_headers):
        ...
        mock_result = SSHResult(
            stdout="hosts content", stderr="", exit_code=0,
            duration_seconds=0.2, host="pi", command="cat /etc/hosts",
        )
        with patch.object(app_state.ssh_proxy, "execute", new_callable=AsyncMock, return_value=mock_result):
            resp = no_approval_client.post(
                "/ssh/exec", json={"host": "pi", "command": "cat /etc/hosts"}, headers=auth_headers,
            )
            assert resp.status_code == 200
            data = resp.json()
