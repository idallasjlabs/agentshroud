...
from security.oauth_security import (
    OAuthSecurityValidator, OAuthRequest, OAuthError,
    ConfusedDeputyError, PKCEViolation, RedirectMismatch
)
...
class TestClientValidation:
    def test_unique_client_id_accepted(self, validator):
        req = OAuthRequest(client_id="client-abc-123", redirect_uri="https://app.example.com/callback",
                           state="random-state", code_challenge="challenge", code_challenge_method="S256")
    ...
    def test_static_shared_client_id_rejected(self, validator):
        ...
        req = OAuthRequest(client_id="public-client", redirect_uri="https://app.example.com/callback",
    ...
    def test_empty_client_id_rejected(self, validator):
        req = OAuthRequest(client_id="", redirect_uri="https://app.example.com/callback",
                           state="s", code_challenge="c", code_challenge_method="S256")
...
class TestStateValidation:
    def test_valid_state_accepted(self, validator):
        req = OAuthRequest(client_id="c1", redirect_uri="https://app.example.com/callback",
                           state="abc123xyz", code_challenge="ch", code_challenge_method="S256")
        assert validator.validate_request(req)

    def test_empty_state_rejected(self, validator):
        req = OAuthRequest(client_id="c1", redirect_uri="https://app.example.com/callback",
                           state="", code_challenge="ch", code_challenge_method="S256")
        with pytest.raises(OAuthError):
            validator.validate_request(req)

    def test_short_state_rejected(self, validator):
        req = OAuthRequest(client_id="c1", redirect_uri="https://app.example.com/callback",
                           state="ab", code_challenge="ch", code_challenge_method="S256")
        with pytest.raises(OAuthError):
            validator.validate_request(req)

    def test_state_replay_detected(self, validator):
        req = OAuthRequest(client_id="c1", redirect_uri="https://app.example.com/callback",
                           state="unique-state-1234", code_challenge="ch", code_challenge_method="S256")
        validator.validate_request(req)
...
class TestPKCE:
    def test_pkce_required_missing_challenge(self, validator):
        req = OAuthRequest(client_id="c1", redirect_uri="https://app.example.com/callback",
                           state="valid-state", code_challenge=None, code_challenge_method=None)
        with pytest.raises(PKCEViolation):
            validator.validate_request(req)

    def test_pkce_s256_accepted(self, validator):
        req = OAuthRequest(client_id="c1", redirect_uri="https://app.example.com/callback",
                           state="valid-state", code_challenge="abcdef", code_challenge_method="S256")
        assert validator.validate_request(req)

    def test_pkce_plain_rejected_when_s256_required(self, validator):
        validator.require_s256 = True
        req = OAuthRequest(client_id="c1", redirect_uri="https://app.example.com/callback",
                           state="valid-state", code_challenge="abcdef", code_challenge_method="plain")
        with pytest.raises(PKCEViolation):
            validator.validate_request(req)
...
class TestRedirectURI:
    ...
    def test_http_rejected(self, validator):
        v = OAuthSecurityValidator(allowed_redirect_uris=["http://localhost/cb"], require_pkce=False)
