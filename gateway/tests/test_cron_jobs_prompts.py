...
_SSH_CONFIG = (
    Path(__file__).parent.parent.parent / "docker" / "config" / "ssh" / "config"
)
...
def test_cron_prompts_warn_against_denied_token():
    ...
    for job in collab:
        ...
        assert "Do NOT use the words" in msg, (
...
def test_cron_prompts_exclude_short_uids():
    ...
    for job in collab:
        msg = job.get("payload", {}).get("message", "")
        assert "shorter than 7 digits" in msg, (
...
def test_ssh_config_routes_tailnet_fqdn():
    ...
    assert tailnet_pos < wildcard_pos, (
...
def test_ssh_tailnet_block_has_proxy_command():
    ...
    assert "ProxyCommand" in tailnet_section, (
        "Tailnet Host block must include a ProxyCommand directive"
    )
    assert "gateway:8181" in tailnet_section, (
        "Tailnet ProxyCommand must route through gateway:8181"
    )
