...
from security.token_validation import (
    TokenValidator, TokenValidationResult, TokenError,
    AudienceMismatch, IssuerMismatch, ScopeViolation, TokenExpiredError
)
...
class TestAudienceValidation:
    def test_correct_audience_accepted(self, validator):
        token = _make_token({"aud": "https://api.agentshroud.com", "iss": "https://auth.agentshroud.com",
                             "exp": time.time() + 3600, "scope": "read"})
        result = validator.validate(token)
        assert result.valid

    def test_wrong_audience_rejected(self, validator):
        token = _make_token({"aud": "https://other-api.com", "iss": "https://auth.agentshroud.com",
                             "exp": time.time() + 3600, "scope": "read"})
        with pytest.raises(AudienceMismatch):
            validator.validate(token)

    def test_missing_audience_rejected(self, validator):
        token = _make_token({"iss": "https://auth.agentshroud.com", "exp": time.time() + 3600, "scope": "read"})
        with pytest.raises(AudienceMismatch):
            validator.validate(token)

    def test_array_audience_accepted(self, validator):
        token = _make_token({"aud": ["https://api.agentshroud.com", "https://other.com"],
...
class TestIssuerValidation:
    def test_wrong_issuer_rejected(self, validator):
        token = _make_token({"aud": "https://api.agentshroud.com", "iss": "https://evil-auth.com",
                             "exp": time.time() + 3600, "scope": "read"})
        with pytest.raises(IssuerMismatch):
            validator.validate(token)

    def test_missing_issuer_rejected(self, validator):
        token = _make_token({"aud": "https://api.agentshroud.com", "exp": time.time() + 3600, "scope": "read"})
        with pytest.raises(IssuerMismatch):
            validator.validate(token)
...
class TestScopeEnforcement:
    def test_requested_scope_within_granted(self, validator):
        token = _make_token({"aud": "https://api.agentshroud.com", "iss": "https://auth.agentshroud.com",
                             "exp": time.time() + 3600, "scope": "read write"})
...
class TestAuditLogging:
    ...
    def test_failed_validation_logged(self, validator):
        token = _make_token({"aud": "https://wrong.com", "iss": "https://auth.agentshroud.com",
                             "exp": time.time() + 3600, "scope": "read"})
        with pytest.raises(AudienceMismatch):
            validator.validate(token)
        logs = validator.get_audit_log()
