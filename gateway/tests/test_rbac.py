...
class TestRBACManager:
    ...
    def test_permission_matrix_viewer(self):
        ...
        assert not self.rbac_manager.check_permission(
            viewer_id, Action.WRITE, Resource.FILES
        ).allowed
        assert not self.rbac_manager.check_permission(
            viewer_id, Action.TOOL_USE, Resource.TOOLS
        ).allowed

        # Viewer should not have admin actions
        assert not self.rbac_manager.check_permission(
            viewer_id, Action.MANAGE, Resource.USERS
        ).allowed
        assert not self.rbac_manager.check_permission(
            viewer_id, Action.SET_ROLE, Resource.USERS
        ).allowed
    ...
    def test_permission_matrix_collaborator(self):
        ...
        assert self.rbac_manager.check_permission(
            collaborator_id, Action.READ, Resource.FILES
        ).allowed
        assert self.rbac_manager.check_permission(
            collaborator_id, Action.WRITE, Resource.FILES
        ).allowed
        assert self.rbac_manager.check_permission(
            collaborator_id, Action.TOOL_USE, Resource.TOOLS
        ).allowed

        # Collaborator should not have admin actions
        assert not self.rbac_manager.check_permission(
            collaborator_id, Action.MANAGE, Resource.USERS
        ).allowed
        assert not self.rbac_manager.check_permission(
            collaborator_id, Action.SET_ROLE, Resource.USERS
        ).allowed
        assert not self.rbac_manager.check_permission(
            collaborator_id, Action.CONFIGURE, Resource.SYSTEM
        ).allowed
    ...
    def test_permission_matrix_admin(self):
        ...
        assert self.rbac_manager.check_permission(
            admin_id, Action.CONFIGURE, Resource.SYSTEM
        ).allowed
        assert self.rbac_manager.check_permission(
            admin_id, Action.APPROVE, Resource.APPROVALS
        ).allowed

        # Admin should not be able to set roles (owner-only)
        assert not self.rbac_manager.check_permission(
            admin_id, Action.SET_ROLE, Resource.USERS
        ).allowed
    ...
    def test_tool_tier_permissions(self):
        ...
        assert self.rbac_manager.check_tool_permission(
            self.collaborator_id, ToolTier.MEDIUM
        ).allowed
        assert not self.rbac_manager.check_tool_permission(
            self.collaborator_id, ToolTier.HIGH
        ).allowed
        assert not self.rbac_manager.check_tool_permission(
            self.collaborator_id, ToolTier.CRITICAL
        ).allowed
...
class TestRBACIntegration:
    ...
    async def test_rbac_handles_missing_user_id(self):
        ...
        request_data = {"message": "some request without user id"}
