...
_INJECTION_PATTERNS: list[tuple[str, re.Pattern, float, str]] = [
    ...
    ("hidden_instruction_marker", re.compile(
        ...
        re.IGNORECASE), 0.6, "Hidden instruction marker"),
]
...
class WebContentScanner:
        ...
        for name, pattern, weight, desc in _INJECTION_PATTERNS:
            ...
            if matches:
                ...
                result.findings.append(ContentFinding(
                    ...
                    severity=FindingSeverity.HIGH if weight >= 0.8 else FindingSeverity.MEDIUM,
                    description=desc,
    ...
    def _scan_hidden_content(self, content: str, result: ScanResult) -> None:
        ...
        hidden_patterns = [
            re.compile(r'style\s*=\s*["\'][^"\']*(?:display\s*:\s*none|visibility\s*:\s*hidden|font-size\s*:\s*0|opacity\s*:\s*0|height\s*:\s*0|width\s*:\s*0)[^"\']*["\'][^>]*>(.*?)<', re.IGNORECASE | re.DOTALL),
            re.compile(r'<\s*(?:div|span|p)\s+[^>]*hidden[^>]*>(.*?)<', re.IGNORECASE | re.DOTALL),
        ]
        ...
        meta_tags = re.findall(r'<meta\s+[^>]*content\s*=\s*["\']([^"\']+)["\']', content, re.IGNORECASE)
    ...
    def _scan_encoded_payloads(self, content: str, result: ScanResult) -> None:
        ...
        b64_matches = re.findall(r'(?:atob|base64[_,]?decode|fromCharCode)\s*\(\s*["\']([A-Za-z0-9+/=]{20,})["\']', content, re.IGNORECASE)
        ...
        standalone_b64 = re.findall(r'(?<![A-Za-z0-9+/=])([A-Za-z0-9+/]{50,}={0,2})(?![A-Za-z0-9+/=])', content)
    ...
    def _scan_pii(self, content: str, result: ScanResult) -> None:
        ...
        for pii_type, pattern in _RESPONSE_PII_PATTERNS:
            ...
            if matches:
                ...
                result.findings.append(ContentFinding(
                    category="pii",
                    severity=FindingSeverity.MEDIUM if pii_type in ("email", "phone") else FindingSeverity.HIGH,
