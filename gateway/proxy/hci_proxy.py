...
"""HCI (Hermes Control Interface) Reverse Proxy with HTTP Basic Auth Gate.

Intercepts all HCI traffic on port 9121 and validates the gateway password
via HTTP Basic Auth before forwarding to the agentshroud-hci container on
the agentshroud-isolated network (http://hci:3000).

Architecture:
  Host :9121 → [gateway hci_proxy] → hci:3000 (agentshroud-isolated network)
...
logger = logging.getLogger("agentshroud.proxy.hci")

_BOT_HCI_URL = os.environ.get("HCI_BOT_URL", "http://hci:3000")
# Skip HTTP Basic Auth on the HCI proxy (e.g. for local dev).
# The port is already bound to 127.0.0.1 and served only over Tailscale.
_SKIP_BASIC_AUTH = os.environ.get("HCI_SKIP_BASIC_AUTH", "0") == "1"
...
def _check_basic_auth(authorization_header: str) -> bool:
    ...
    try:
        ...
        if not expected:
            logger.error("HCI proxy: gateway password unavailable — denying all requests")
            return False
...
async def hci_proxy_app(scope: dict[str, Any], receive: Any, send: Any) -> None:
    """ASGI application: auth-gated transparent reverse proxy for HCI.
...
async def _handle_http(scope: dict[str, Any], receive: Any, send: Any) -> None:
    ...
    if not _SKIP_BASIC_AUTH and not _check_basic_auth(auth_header):
        logger.info(
            "HCI proxy: unauthorized HTTP request from %s %s",
            scope.get("client"),
            scope.get("path"),
        )
        ...
        await send(
            {
                ...
                "status": 401,
                "headers": [
                    [b"www-authenticate", b'Basic realm="AgentShroud HCI"'],
    ...
    try:
        ...
        logger.error("HCI proxy: upstream request failed: %s", exc)
        error_body = b"HCI unavailable"
        await send(
            {
                "type": "http.response.start",
                "status": 502,
...
async def _handle_websocket(scope: dict[str, Any], receive: Any, send: Any) -> None:
    ...
    if not _SKIP_BASIC_AUTH and not _check_basic_auth(auth_header):
        logger.info(
            "HCI proxy: unauthorized WebSocket from %s %s",
            scope.get("client"),
            scope.get("path"),
        )
    ...
    qs = qs_raw
    ws_url = _BOT_HCI_URL.replace("http://", "ws://").replace("https://", "wss://") + path
    if qs:
    ...
    # Forward Host header to HCI
    origin = headers.get("origin", "")
    extra_headers = {"Host": "localhost:3000"}
    if origin:
        extra_headers["Origin"] = origin

    try:
        async with websockets.connect(
            ws_url, additional_headers=extra_headers, max_size=10 * 1024 * 1024
        ) as upstream_ws:
        ...
        logger.warning("HCI proxy WebSocket upstream error: %s", exc)
    except Exception as exc:
        logger.error("HCI proxy WebSocket error: %s", exc)
    finally:
        try:
