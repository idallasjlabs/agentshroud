...
class MCPInspector:
    ...
    def _normalize_unicode(text: str) -> str:
        ...
        to their ASCII equivalents so injection patterns cannot be bypassed
    ...
    def inspect_tool_result(
        ...
        Focus: PII leakage, injection patterns, and suspicious encoding in responses.
        We do NOT block responses (the call already happened) — we redact and log.
        
        CVE-2026-22708: Tool results from MCP servers can contain hidden prompt 
        injection that flows into agent context. FLAG but don't block since the 
        ...
        for text, path in strings:
            if check_injection:
                # CRITICAL: Scan for prompt injection in tool results (CVE-2026-22708)
                # Results are flagged but not blocked since the call already happened
                findings.extend(self._check_injection(text, path))
            if check_pii:
                findings.extend(self._check_pii(text, path))
            if check_encoding:
        ...
        result = InspectionResult(
            blocked=False,  # Never block responses, just redact and log
            findings=findings,
