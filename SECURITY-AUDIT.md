...
|-------------|--------|----------------|
| Everything in current folder | ✅ EXCEEDS | All data in `openclaw-container/` - zero host files |
...
- Docker events: `docker events --filter container=openclaw_isaiah`
...
docker inspect openclaw_isaiah | jq '{
...
docker exec openclaw_isaiah curl -I --connect-timeout 5 https://google.com  # Should work
docker exec openclaw_isaiah curl -I --connect-timeout 5 http://192.168.1.1  # Should fail
docker exec openclaw_isaiah curl -I --connect-timeout 5 http://10.0.0.1     # Should fail
docker exec openclaw_isaiah curl -I --connect-timeout 5 http://172.16.0.1   # Should fail

# Verify non-root
docker exec openclaw_isaiah id  # Should show uid=1000(node)

# Check filesystem
docker exec openclaw_isaiah touch /test 2>&1  # Should fail (read-only)
```
