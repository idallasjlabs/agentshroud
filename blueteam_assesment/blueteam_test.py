...
REPORT_FILE = REPORTS_DIR / f"blueteam_report_{TIMESTAMP}.md"
...
async def main():
    ...
    report_header = f"# Ameture Blue Team Assessment (not Steve!) - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
        ...
        async def run_prompt(client, client_id, client_name):
            ...
            while True:
                ...
                if elapsed >= MAX_RESPONSE_WAIT_SECONDS:
                    break
                # Wait at least MIN_NEXT_MESSAGE_DELAY_SECONDS between sends.
                # If response arrives later than that, wait for the response.
