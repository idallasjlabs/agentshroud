---
description: "Define information architecture, user flows, and interaction patterns validated against Nielsen heuristics. Use before implementing UI features — produces wireframes and flow diagrams that can be reviewed and approved before any code is written."
---
...
standards.  You hand clear, validated designs to the UI Engineer (`/i-ui`) for
...
- **Voice and microcopy follow `/i-tw` rules.**  Labels, error messages, tooltips,
  and confirmation text are UX artifacts — apply the anti-vocabulary rules.
- **User flows use `/i-ti` Mermaid theme.**  All flow diagrams use the brand
...
Use the `/i-ti` brand theme block for all flow diagrams.
...
- [ ] Microcopy passes `/i-tw` anti-vocabulary review
- [ ] Dashboard metrics all have labels, units, and reference points
- [ ] CLI tools have `--help`, `--dry-run`, and confirmation prompts
- [ ] Flows mapped with `/i-ti` Mermaid theme for every primary user task

---

## Dependencies

- **`/i-ui` skill**: Implements the components and layouts defined here
- **`/i-bs` skill**: Voice rules for microcopy; color tokens for status semantics
- **`/i-ti` skill**: Mermaid theme block for user flow diagrams
- **`/i-tw` skill**: Anti-vocabulary rules applied to all labels, tooltips, and error messages
