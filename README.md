...
<p>
  ...
  <img src="https://img.shields.io/badge/modules-75-blueviolet" alt="75 Security Modules" />
</p>
...
v1.2.0 adds the **ESP32-S3 Voice Terminal** — an optional hardware control surface for AgentShroud. Speak to your agents from anywhere via a pocket-sized device; all audio routes through the same 75-module security pipeline.
...
The v1.1.x line introduces **multi-bot governance**: AgentShroud now secures two autonomous AI agents simultaneously — **OpenClaw** (Node.js, the original) and **Hermes** (Python, `nousresearch/hermes-agent`) — through the same 75-module security pipeline.
...
The original release wired all **75 security modules** into the live pipeline and delivered two complete control center interfaces — Web (7-page responsive dashboard) and Terminal (TUI + chat console). Every module is active in the pipeline — no stubs, no dead code, no planned-but-unbuilt features.
...
**Multi-bot**: v1.1.0 introduces a second bot — Hermes — running alongside OpenClaw behind the same 75-module gateway. Start the full stack (gateway + OpenClaw + Hermes + HCI) with `--profile full`; omit `--profile full` for gateway + OpenClaw only.
...
AgentShroud implements a defense-in-depth strategy with 75 security modules operating across 7 distinct layers, from network isolation to application-level content filtering.
...
| **SecurityPipelineIntegrator** | Orchestrates all 75 modules into a unified processing pipeline |
...
- **Security Modules** — Status and configuration for all 75 modules
