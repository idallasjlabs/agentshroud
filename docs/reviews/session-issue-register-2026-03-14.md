...
|---:|---|---|---|---|---|
| 1 | Collaborator Access | Collaborators unable to join reliably via `/start` | In Progress | Inbound tests + manual `/start` | Medium |
| 2 | Collaborator Access | Pending collaborator stuck in approval state | In Progress | Pending/approve flow tests | Medium |
| 3 | Collaborator Access | Existing collaborators unexpectedly removed/revoked | Open | Manual roster checks (`/collabs`) | Medium |
| 4 | Collaborator Access | Need owner revoke/retest flow | Implemented | Owner command tests (`/revoke`) | Low |
| 5 | Collaborator Access | Need manual re-add for known collaborators | In Progress | `/addcollab` + `/restorecollabs` checks | Medium |
| 6 | Collaborator Access | `/whoami` intermittently failed | In Progress | Local command normalization tests | Medium |
| 7 | Collaborator Access | Need mapping from Telegram IDs to real users | In Progress | `/collabs` + label resolution checks | Medium |
...
| 10 | No Response | Collaborator messages got no response | In Progress | assessment reports + fallback tests | High |
| 11 | No Response | Owner responds while collaborator gets silence | In Progress | dual-account manual runs | High |
...
| 30 | Command Contract | `/status`, `/help`, `/start` intermittent failures | In Progress | local command/fallback tests | Medium |
...
| 49 | Runtime | Gateway/container restart loops | In Progress | rebuild + health logs | Medium |
| 50 | Runtime | “Online” sent before truly ready | In Progress | startup sequencing checks | Medium |
| 51 | Runtime | Need starting→online readiness semantics | In Progress | startup message tests/manual | Low |
