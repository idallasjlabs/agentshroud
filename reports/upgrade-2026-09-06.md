...
fabricated; see "Not done" at the bottom.
...
1. **HIGH PRIORITY — voice_gateway CVE exposure is real, not theoretical.**
   ...
   `ffmpeg`/`libav*`, which currently carry 153 unfixed HIGH/CRITICAL CVEs on
   Debian trixie with no patches available. Recommend evaluating a rebase of
   `voice_gateway/Dockerfile`'s `FROM python:3.12-slim@sha256:c3d81d25…` to
   the `-bookworm` variant (`python:3.12-slim-bookworm`, confirmed still
   published) instead of trixie — bookworm has active `bookworm-security`
   backport coverage (visible in the OpenClaw build's own apt sources) that
   trixie currently lacks. **Not done here** — swapping a Debian release
   underneath a working audio pipeline is a real behavioral-risk change
   (dependency ABI, ffmpeg build flags/codec availability) that deserves its
   own dedicated test pass, not a Sunday drive-by.
...
**OpenClaw base-image bump (the only live-stack change made):**
```bash
...
Approximately 75 minutes of active work (recon fork + this session),
within the 90-minute budget referenced by the generic mission template
(not strictly binding here since this was a user-directed scoped task, not
the full autonomous procedure).
