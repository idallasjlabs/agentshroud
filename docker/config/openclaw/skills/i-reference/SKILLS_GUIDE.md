...
|----------------|-------|---------|
| Starting a task | `/i-gg` | Branch naming, workflow rules |
| Writing code | `/i-tdd` | TDD discipline (Red-Green-Refactor) |
| Testing in prod | `/i-qa` | Production testing procedures |
| Data validation | `/i-data` | Athena queries, data quality |
| Using MCP tools | `/i-mcpm` | GitHub, Jira, AWS integrations |
| MCP diagnostics | `/i-mcpm-doctor` | Diagnose MCP server issues |
| MCP auth reset | `/i-mcpm-auth-reset` | Reset MCP authentication |
| AWS MCP profile | `/i-mcpm-aws-profile` | Configure AWS MCP profile |
| Code review | `/i-cr` | Review checklist, security |
| Creating PR | `/i-pr` | PR description template |
| Before merge | `/i-ps` | Pre-deploy checklist |
| Incident response | `/i-production` | Rollback procedures |
| AWS operations | `/i-aws` | Cloud inventory, cost optimization, FinOps |
| Root cause analysis | `/8d` | BESS incident investigation, 8D methodology |
| Mac app inventory | `/i-mac` | Discover all macOS applications |
| Security review | `/i-sec` | SecureClaw 4-layer security review |
| Defensive security | `/i-sec-defense` | Blue team STPA-Sec audit |
| Offensive security | `/i-sec-offense` | Red team adversarial testing |
| Environment setup | `/env` | SecureClaw Pi environment management |
| Project tracking | `/i-pm` | SecureClaw roadmap and sprint tracking |
| Technical writing | `/i-tw` | Documentation authoring |
| Technical diagrams | `/i-ti` | Mermaid diagram generation |
| Branding | `/i-bs` | Visual brand consistency |
| Quick reference | `/i-mc` | Combined checklist |
| Browser automation | `/i-browser` | Playwright browser automation with security controls |
| iCloud services | `/i-icloud` | Calendar, Contacts, Mail via CalDAV/CardDAV |
| Podcast audio | `/i-apollo` | Convert scripts to audio via ElevenLabs |
| Podcast notes | `/i-athena` | Show notes and cheat sheets |
| Podcast curriculum | `/i-atlas` | Episode learning architecture |
| Podcast diagrams | `/i-daedalus` | PlantUML/Mermaid concept illustrations |
| Podcast references | `/i-hermes` | Fact-check and reference verification |
| Podcast retention | `/i-mnemosyne` | Spaced repetition study materials |
| Podcast feedback | `/i-oracle` | Episode quality analysis |
| Podcast dialogue | `/i-socrates` | Curriculum-to-dialogue script writing |
| Podcast audit | `/i-vulcan` | Technical accuracy quality gate |
| System audit docs | `/i-sad` | 13-section exhaustive codebase documentation |
| System audit vault | `/i-sav` | Complete Obsidian vault for a codebase |
...
### Core Development
- **QA** (`/i-qa`) - Production testing procedures, service control, DB test patterns
- **CR** (`/i-cr`) - Code review checklist with production safety checks
- **PR** (`/i-pr`) - PR description generator with rollback plans

### Workflow
- **TDD** (`/i-tdd`) - Test-driven development discipline
- **GIT-GUARD** (`/i-gg`) - GitHub workflow enforcement
- **CICD** (`/i-cicd`) - CI/CD pipeline design

### Production Safety
- **PROD-SAFETY** (`/i-ps`) - Pre/post-deployment checklists
- **INCIDENT** (`/i-production`) - Incident response and rollback

### Data Operations
- **DATA-VAL** (`/i-data`) - Data quality validation with cost control

### Cloud & FinOps
- **AWS** (`/i-aws`) - AWS cloud inventory, cost optimization, FY26 40% reduction plan, script templates (bash/python), tagging governance, EBS/RDS/S3 rightsizing
...
### System Utilities
- **MAC** (`/i-mac`) - macOS application inventory (10 collection methods), categorization (22 categories), JSON manifest + Markdown catalog, web-sourced alternatives

### SecureClaw (Project-Specific)
- **SEC** (`/i-sec`) - 4-layer security review (application, container, network, data flow), threat model
- **ENV** (`/env`) - Raspberry Pi environment management, Docker hardening, ARM64 considerations, CI/CD workflow
- **PM** (`/i-pm`) - 8-phase roadmap tracking, continuity files, sprint management, 6-agent coordination

### MCP Tools
- **MCPM** (`/i-mcpm`) - GitHub, Jira, AWS MCP integration
- **MCPM-DOCTOR** (`/i-mcpm-doctor`) - Diagnose MCP server issues
- **MCPM-AUTH-RESET** (`/i-mcpm-auth-reset`) - Reset MCP authentication
- **MCPM-AWS-PROFILE** (`/i-mcpm-aws-profile`) - Configure AWS MCP profile

### Content & Communication
- **TW** (`/i-tw`) - Technical documentation authoring: READMEs, runbooks, ADRs, API reference
- **TI** (`/i-ti`) - Mermaid diagram generation: architecture, data flow, sequence, ER, state diagrams
- **BS** (`/i-bs`) - Brand consistency: color tokens, typography, diagram themes, voice & tone

### Reference
- **MASTER** (`/i-mc`) - Combined quick-reference checklist
...
3. **Always cleanup and re-enable**
   ...
   - Verify cleanup with checklist (`/i-qa` Section H.4)
...
3. **Rollback** - Use `/i-production` for detailed rollback procedures
