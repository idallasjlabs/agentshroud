---
description: "Structure complex knowledge hierarchically as XMind or Markmap mind maps with visual hierarchy best practices. Use when planning a complex change with many moving parts, mapping dependencies, or communicating system structure to stakeholders before implementation."
---
...
- **Complement, do not duplicate, existing diagrams.**  Mindmaps show structure and
  relationships.  Sequence and flow diagrams (via `/i-ti`) show process and time.
...
| "How does X work step by step?" | Flowchart (`/i-ti`) | Sequential, ordered |
| "What is the order of events?" | Timeline / Gantt | Time-ordered |
| "How do A and B relate?" | ER / graph diagram (`/i-ti`) | Relational |
...
| "What decisions were made?" | ADR document (`/i-tw`) | Narrative, traceable |
| "How do systems communicate?" | Sequence diagram (`/i-ti`) | Protocol, time-ordered |
...
- **`/i-ti` skill**: For diagrams that require process flow, sequence, or ER representation
- **`/i-atlas` skill**: Curriculum design — mindmaps complement episode structure outlines
- **`/i-tw` skill**: Noun-phrase label standards follow the same precision rules as technical writing
