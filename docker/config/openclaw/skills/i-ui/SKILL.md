---
description: "Implement UI components using brand design tokens, CSS architecture patterns, and WCAG AA accessibility standards. Use for frontend component work requiring consistent styling, accessible markup, and design system compliance."
---
...
`/i-bs` as your only source of truth for color, typography, and spacing.
...
## Rules
- **Consume `/i-bs` tokens — never redefine them.**  If a token doesn't exist,
...
| Color contrast | All text pairs pass WCAG AA (4.5:1 text, 3:1 UI) — validated by `/i-bs` |
...
- [ ] WCAG AA contrast verified (use `/i-bs` contrast validation command)

---

## Dependencies

- **`/i-bs` skill**: Source of truth for all design tokens — color, typography, spacing
- **`/i-ux` skill**: Defines information architecture and interaction patterns that UI implements
...
- **`@mermaid-js/mermaid-cli`**: `npm install -g @mermaid-js/mermaid-cli` — for `/i-ti` diagrams
