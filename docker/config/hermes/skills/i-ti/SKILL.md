---
description: "Produce accurate Mermaid diagrams from code with brand theme applied; validates diagram accuracy against source before exporting SVG. Use when documenting data flows, system architecture, or sequence diagrams for PRs, runbooks, or Confluence pages."
---
...
- **Brand theme mandatory.**  Every diagram starts with the init block below.  Never
  override individual node colors — change the token in `/i-bs` and propagate.
...
Never modify individual colors here — update `/i-bs` and copy the new block.
...
- **`/i-bs` skill**: Source of truth for brand theme block — copy from there, not here
- **`/i-tw` skill**: Doc structure rules — diagrams are referenced by link, not embedded as prose
